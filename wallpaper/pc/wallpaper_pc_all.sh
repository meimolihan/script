...
# ==================== Gitee 脚本库 URL ====================
GITEE_SCRIPT_BASE_URL="https://gitee.com/meimolihan/script/raw/master/wallpaper/pc"

# ==================== 目录变量定义 ====================
PHOTOS_DIR="/vol1/1000/compose/random-pic-api/photos" # 待处理目录
BACKUP_DIR="/vol2/1000/阿里云盘/教程文件/壁纸原图/电脑原图" # 壁纸原图目录
LANDSCAPE_DIR="/vol1/1000/compose/random-pic-api/landscape" # 壁纸目录
...
# 备份原图函数
backup_photos() {
    if ! confirm_action "是否备份电脑壁纸原图" "n"; then
...
echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
execute_remote_script "wallpaper_pc_dirs.sh"
...
if check_and_cd "$LANDSCAPE_DIR" "无法进入目录: $LANDSCAPE_DIR"; then
    ...
    execute_remote_script "rename_webp_pc.sh"
fi
...
if check_and_cd "$BACKUP_DIR" "无法进入目录: $BACKUP_DIR"; then
    ...
    execute_remote_script "rename_webp-png-jpg_pc.sh"
fi
