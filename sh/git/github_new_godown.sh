...
echo -e "${BLUE}==================================${RESET}"
echo -e "${YELLOW}注意事项："
echo -e "1. 请确保已在 GitHub 创建名为 ${repo_dir} 的空仓库"
...
# 设置远程仓库
remote_url="https://github.com/meimolihan/${repo_name}.git"
...
echo.
echo -e "${BLUE}==================================${RESET}"
echo -e "${GREEN}正在将远程仓库修改为 SSH 连接...${RESET}"
echo -e "${BLUE}==================================${RESET}"
# 检查当前目录是否是一个 Git 仓库
if ! git rev-parse --git-dir > /dev/null 2>&1; then
    echo -e "${GREEN}当前目录不是一个 Git 仓库。${RESET}"
    exit 1
fi

echo -e "${GREEN}当前目录是一个 Git 仓库。${RESET}"
echo -e "${BLUE}==================================${RESET}"
...
while IFS= read -r line; do
    ...
    if [[ "$remote_url" =~ ^https://(gitee|github)\.com/ ]]; then
        ...
        echo -e "${GREEN}将远程仓库 $remote_name 的 URL 从 $remote_url 修改为 $ssh_url${RESET}"
        git remote set-url "$remote_name" "$ssh_url"
    else
        echo -e "${GREEN}远程仓库 $remote_name 的 URL 已经是 SSH 格式，无需修改。${RESET}"
    fi
done <<< "$remote_info"

echo -e "${GREEN}远程仓库已更新为 SSH 连接。${RESET}"
echo -e "${GREEN}==================================${RESET}"
echo -e "${GREEN}仓库初始化并推送成功！${RESET}"
echo -e "${GREEN}仓库地址：https://github.com/meimolihan/${repo_name}${RESET}"
echo -e "${GREEN}==================================${RESET}"
