...
echo -e "${gl_zi}>>> GitHub 新仓库初始化脚本${gl_bai}"
echo -e "${gl_bufan}------------------------${gl_bai}"
echo -e "当前工作目录: ${gl_huang}$(pwd)${gl_bai}"
echo -e "${gl_bufan}------------------------${gl_bai}"
echo -e "${gl_huang}注意事项："
echo -e "${gl_bufan}1. ${gl_bai}请确保已在 GitHub 创建名为 ${gl_huang}$repo_dir${gl_bai} 的空仓库"
echo -e "${gl_bufan}2. ${gl_bai}需要提前配置好 SSH 密钥"
echo -e "${gl_bufan}3. ${gl_bai}前往 GitHub 创建：${gl_lv}https://github.com/meimolihan${gl_bai}"
echo -e "${gl_bufan}------------------------${gl_bai}"
...
# 设置远程仓库
remote_url="git@github.com:meimolihan/${repo_name}.git"
...
# 推送代码
echo -e "${gl_bufan}正在推送代码到 GitHub...${gl_bai}"
...
echo -e "${gl_lv}仓库地址：https://github.com/meimolihan/${repo_name}${gl_bai}"
echo -e "${gl_lv}==================================${gl_bai}"
