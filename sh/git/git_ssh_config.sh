...
# 欢迎信息
print_divider
print_info "欢迎使用 Git SSH 配置脚本！"
print_info "此脚本将自动将当前目录设置为 Git 安全目录，并将远程仓库的连接方式修改为 SSH。"
print_divider
...
while IFS= read -r line; do
    ...
    if [[ "$remote_url" =~ ^https://(gitee|github)\.com/ ]]; then
        ...
        print_info "将远程仓库 $remote_name 的 URL 从 $remote_url 修改为 $ssh_url"
        git remote set-url "$remote_name" "$ssh_url"
    else
        print_info "远程仓库 $remote_name 的 URL 已经是 SSH 格式，无需修改。"
    fi
done <<< "$remote_info"

print_info "远程仓库已更新为 SSH 连接。"
print_divider
