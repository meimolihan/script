...
#   3. 遍历当前目录下的所有子目录，将每个子目录压缩成单独的 zip 文件并保存到备份目录。
#   4. 提供详细的错误处理和用户反馈。
# 使用方法：
#   1. 将脚本保存为 backup-zip.sh。
#   2. 赋予执行权限：chmod +x backup-zip.sh。
#   3. 运行脚本：./backup-zip.sh。
#   4. 按照提示选择备份路径或输入自定义路径。
# 注意事项：
#   - 确保运行脚本的用户对备份目录有写权限。
#   - 当前目录下的隐藏文件夹（以 . 开头）不会被备份。
#   - 需要安装 zip 工具：sudo apt-get install zip
#   - 备份文件名格式：文件夹名字-yyyy-mm-dd.zip
# =================================================================================================

clear
echo ==================================
echo 功能描述：
echo   1. 显示备份路径选项菜单，包括预定义路径和自定义路径选项。
echo   2. 根据用户选择，创建备份目录（如果不存在）。
echo   3. 遍历当前目录下的所有子目录，将每个子目录压缩成单独的 zip 文件并保存到备份目录。
echo   4. 提供详细的错误处理和用户反馈。
echo 注意事项：
echo   1.确保运行脚本的用户对备份目录有写权限。
echo   2.当前目录下的隐藏文件夹（以 . 开头）不会被备份。
echo   3.需要安装 zip 工具：sudo apt-get install zip
echo   4.备份文件名格式：文件夹名字-yyyy-mm-dd.zip
echo ==================================
echo
# 设置预定义的备份路径选项
options=(
...
while true; do
    ...
    for d in */; do
        ...
        # 使用当前日期作为备份文件名的一部分
        zip -r "${backup_dir}/${dir_name}-${current_date}.zip" "$dir_name" || {
            echo -e "\033[31m错误：无法备份 ${dir_name}！\033[0m"
            continue
        }
        echo -e "\033[32m成功备份 ${dir_name} 到 ${backup_dir}/${dir_name}-${current_date}.zip\033[0m"
    done
