...
sh_download() {
    ...
    # 4. 根据系统类型创建快捷命令
    if [ "$ISTOREOS" = 1 ]; then
        # iStoreOS：alias 方式，写入 /etc/profile.d/istoreos-g.sh 保证登录自动生效
        echo "alias g='$script_path'" > /etc/profile.d/istoreos-g.sh
        chmod +x /etc/profile.d/istoreos-g.sh
        # 当前 shell 立即生效
        alias g="$script_path"
    else
        # 非 iStoreOS：软链到 /usr/local/bin
        ln -sf "$script_path" /usr/local/bin/g
    fi
