...
linux_file() {
	...
	while true; do
		...
		echo "------------------------"
		echo "11. 创建文件           12. 编辑文件             13. 创建并编辑文件       14. 修改文件权限"
		echo "15. 删除文件           16. 重命名文件"
		echo "------------------------"
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}gitee.com/meimolihan/script/raw/master/sh/tool/mobufan_sh_log.txt"
	echo "------------------------"

	curl -s ${gh_proxy}gitee.com/meimolihan/script/raw/master/sh/tool/mobufan_sh_log.txt | tail -n 30
	local sh_v_new=$(curl -s ${gh_proxy}gitee.com/meimolihan/script/raw/master/sh/tool/mobufan.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local country=$(curl -s ipinfo.io/country)
			if [ "$country" = "CN" ]; then
				curl -sS -O ${gh_proxy}gitee.com/meimolihan/script/raw/master/sh/install/mobufan.sh && chmod +x mobufan.sh
			else
				curl -sS -O ${gh_proxy}gitee.com/meimolihan/script/raw/master/sh/install/mobufan.sh && chmod +x mobufan.sh
			fi
			canshu_v6
...
show_compose_project_menu() {
    ...
    if [ ! -d "$base_path" ]; then
        echo -e "${gl_huang}错误: 路径 $base_path 不存在${NC}"
        echo -e "${gl_bufan}按任意键继续...${NC}"
        read -n 1 -s
        return
    fi
...
linux_fnos_menu() {
	while true; do
	  ...
	  echo "FnOS 命令"
          ...
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_bufan}2.   ${gl_bai}当前目录Compose启动服务"
          echo -e "${gl_bufan}3.   ${gl_bai}当前目录Compose停止服务"
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_bufan}4.   ${gl_bai}查看所有Docker容器"
          echo -e "${gl_bufan}5.   ${gl_bai}停止指定Docker容器"
          echo -e "${gl_bufan}6.   ${gl_bai}启动指定Docker容器"
          echo -e "${gl_bufan}7.   ${gl_bai}重启指定Docker容器"
          echo -e "${gl_bufan}------------------------${gl_bai}"
	  ...
	  case $sub_choice in
		  ...
		  5)
                         ...
                         if safe_read "请输入要停止的容器名称或ID(输入q退出)" container; then
                            if [ -n "$container" ]; then
                               echo -e "${gl_bufan}停止容器: $container"
                               docker stop $container
                           else
                                echo -e "${gl_huang}错误: 未输入容器名称或ID"
                             fi
                         fi
                        break_end
                         ;;
		  ...
		  6)
                         ...
                         if safe_read "请输入要启动的容器名称或ID(输入q退出)" container; then
                             if [ -n "$container" ]; then
                                 echo -e "${gl_bufan}启动容器: $container"
		  ...
		  7)
                         ...
                         if safe_read "请输入要重启的容器名称或ID(输入q退出)" container; then
                             if [ -n "$container" ]; then
                                 echo -e "${gl_bufan}重启容器: $container"
...
nginx_conf_manager() {
    ...
    if [ ! -d "$nginx_conf_dir" ]; then
        echo -e "${gl_hong}错误: Nginx 配置目录 $nginx_conf_dir 不存在${gl_bai}"
        return 1
    fi
    ...
    while IFS= read -r -d $'\0' file; do
        conf_files+=("$file")
    done < <(find "$nginx_conf_dir" -maxdepth 1 -name "*.conf" -type f -print0 | sort -z)
    
    # 检查是否有配置文件
    if [ ${#conf_files[@]} -eq 0 ]; then
        echo -e "${gl_huang}在 $nginx_conf_dir 目录下未找到 .conf 文件${gl_bai}"
        return 1
    fi
    
    echo -e "${gl_bufan}Nginx 配置文件管理${gl_bai}"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    echo -e "${gl_bufan}找到 ${#conf_files[@]} 个配置文件:${gl_bai}"
    echo ""
    
    # 计算两列显示的相关参数
    local total_files=${#conf_files[@]}
    local column_count=2
    local rows_per_column=$(( (total_files + column_count - 1) / column_count ))
    
    # 两列显示文件列表
    for ((i=0; i<rows_per_column; i++)); do
        local line=""
        
        for ((col=0; col<column_count; col++)); do
            local index=$((i + col * rows_per_column))
            
            if [ $index -lt $total_files ]; then
                local file_name=$(basename "${conf_files[$index]}")
                local file_size=$(du -h "${conf_files[$index]}" | cut -f1)
                local display_num=$((index + 1))
                
                # 格式化显示，确保对齐
                if [ $col -eq 0 ]; then
                    # 第一列
                    printf "  ${gl_huang}%2d.${gl_bai} %-25s ${gl_bufan}(%4s)${gl_bai}" \
                        "$display_num" "$file_name" "$file_size"
                else
                    # 第二列
                    printf "  ${gl_huang}%2d.${gl_bai} %-25s ${gl_bufan}(%4s)${gl_bai}" \
                        "$display_num" "$file_name" "$file_size"
    ...
    echo ""
    echo -e "${gl_bufan}0. 返回上一级选单${gl_bai}"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    
    # 使用增强的安全读取函数选择文件
    while true; do
        if ! safe_read_enhanced "请选择文件序号 (0-${#conf_files[@]})" selected_num; then
            return
        fi
        ...
        # 检查退出条件
        if [ "$selected_num" -eq 0 ]; then
            echo -e "${gl_huang}返回上一级选单${gl_bai}"
            return
        fi
        
        # 检查序号是否在有效范围内
        if [ "$selected_num" -gt 0 ] && [ "$selected_num" -le "${#conf_files[@]}" ]; then
            selected_file="${conf_files[$((selected_num-1))]}"
            break
        else
            echo -e "${gl_hong}错误: 序号 $selected_num 不在有效范围内 (1-${#conf_files[@]})${gl_bai}"
        fi
    done
    
    # 显示选中的文件信息
    echo ""
    echo -e "${gl_bufan}已选择文件: ${gl_huang}$(basename "$selected_file")${gl_bai}"
    echo -e "${gl_bufan}完整路径: ${gl_bai}$selected_file"
    echo -e "${gl_bufan}文件大小: ${gl_bai}$(du -h "$selected_file" | cut -f1)"
    echo -e "${gl_bufan}修改时间: ${gl_bai}$(stat -c %y "$selected_file" 2>/dev/null | cut -d'.' -f1 || echo "未知")"
    echo "----------------------------------------"
    
    # 显示文件内容预览
    echo -e "${gl_bufan}文件内容预览 (前20行):${gl_bai}"
    echo "----------------------------------------"
    if [ -s "$selected_file" ]; then
        head -n 20 "$selected_file" | cat -n 2>/dev/null || echo -e "${gl_hong}无法读取文件内容${gl_bai}"
        local total_lines=$(wc -l < "$selected_file" 2>/dev/null || echo 0)
        if [ "$total_lines" -gt 20 ]; then
            echo -e "${gl_huang}... (还有 $((total_lines-20)) 行)${gl_bai}"
        fi
    else
        echo -e "${gl_huang}(空文件)${gl_bai}"
    fi
    ...
    # 选择操作
    echo -e "${gl_bufan}请选择操作:${gl_bai}"
    echo -e "  ${gl_huang}1.${gl_bai} 查看完整文件 (cat)"
    echo -e "  ${gl_huang}2.${gl_bai} 编辑文件 (nano)"
    echo -e "  ${gl_huang}3.${gl_bai} 返回文件列表"
    echo -e "  ${gl_huang}0.${gl_bai} 返回主菜单"
    echo "----------------------------------------"
    
    while true; do
        if ! safe_read_enhanced "请选择操作 (0-3)" action; then
            return
        fi
        
        case "$action" in
            0)
                echo -e "${gl_huang}返回主菜单${gl_bai}"
                return
                ;;
            1)
                echo -e "${gl_bufan}查看文件: $selected_file${gl_bai}"
                echo "----------------------------------------"
                if [ -s "$selected_file" ]; then
                    cat -n "$selected_file" 2>/dev/null || echo -e "${gl_hong}无法查看文件内容${gl_bai}"
                else
                    echo -e "${gl_huang}(空文件)${gl_bai}"
                fi
                echo "----------------------------------------"
                ;;
            2)
                echo -e "${gl_bufan}编辑文件: $selected_file${gl_bai}"
                # 检查 nano 是否可用
                if command -v nano >/dev/null 2>&1; then
                    nano "$selected_file"
                    echo -e "${gl_huang}文件编辑完成${gl_bai}"
                    
                    # 编辑后显示文件变化
                    if ! safe_read_confirm "是否查看编辑后的文件" view_after_edit; then
                        break
                    fi
                    if [ "$view_after_edit" = "y" ]; then
                        echo "----------------------------------------"
                        echo -e "${gl_bufan}编辑后的文件内容:${gl_bai}"
                        cat -n "$selected_file" 2>/dev/null || echo -e "${gl_hong}无法读取文件内容${gl_bai}"
                        echo "----------------------------------------"
                    fi
                else
                    echo -e "${gl_hong}错误: nano 编辑器未安装${gl_bai}"
                    if ! safe_read_confirm "是否尝试使用 vi 编辑器" use_vi; then
                        break
                    fi
                    if [ "$use_vi" = "y" ]; then
                        vi "$selected_file"
            ...
            3)
                ...
                ;;
            *)
                echo -e "${gl_hong}错误: 请输入有效的操作序号 (0-3)${gl_bai}"
                ;;
        esac
        
        # 如果不是返回操作，询问是否继续操作当前文件
        if [ "$action" -ne 3 ]; then
            if ! safe_read_confirm "是否继续操作此文件" continue_operation; then
                break
            fi
            if [ "$continue_operation" != "y" ]; then
                if ! safe_read_confirm "是否返回文件列表" return_to_list; then
                    break
                fi
                if [ "$return_to_list" = "y" ]; then
                    nginx_conf_manager
                    return
                else
                    break
...
linux_nginx_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_bufan}7.   ${gl_bai}查看并编辑配置文件 ${gl_huang}★${gl_bai}"
        echo -e "${gl_bufan}8.   ${gl_bai}查看html目录"
        echo -e "${gl_bufan}------------------------${gl_bai}"
...
echo -e "${gl_bufan}命令行输入${gl_huang}m${gl_bufan}可快速启动脚本${gl_bai}"
