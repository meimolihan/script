...
CheckUserAgreement() {
    ...
    echo -e "${gl_lv}首次使用，请先阅读并同意用户许可协议：${gl_bai}"
    echo -e "${gl_lv}https://sh.mobufan.eu.org:666/"
    echo -e "${gl_bufan}----------------------------------------${gl_bai}"
    read -r -rp "是否同意以上条款？(y/n): " input

    if [[ "$input" =~ ^[Yy]$ ]]; then
        echo "agreed" >"$config"
        echo -e "${gl_bufan}感谢你的同意！"
    else
    echo -e "${gl_hong}你必须同意条款才能继续使用。"
        exit 1
    fi
}
...
mobufan_sh_update() {
    local sh_v_new
    sh_v_new=$(curl -s https://gitee.com/meimolihan/script/raw/master/sh/tool/mobufan.sh |
...
docker_ps() {
	...
	echo -e "${gl_bufan}00. ${gl_bai}退出脚本"
        ...
        7)
            ...
            ;;
        8)
            read -r -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定删除所有容器吗？(Y/N): ")" choice
            case "$choice" in
            [Yy])
...
docker_image() {
	...
	echo -e "${gl_bufan}00. ${gl_bai}退出脚本"
        ...
        3)
            ...
            ;;
        4)
            read -r -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定删除所有镜像吗？(Y/N): ")" choice
            case "$choice" in
            [Yy])
...
cf_purge_cache() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        # 提示用户是否清理缓存
        read -r -e -p "需要清理 Cloudflare 的缓存吗？（y/n）: " answer
        if [[ "$answer" == "y" ]]; then
...
web_security() {
    while true; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_bufan}11. ${gl_bai}配置拦截参数                  ${gl_bufan}12. ${gl_bai}清除所有拉黑的IP"
        echo -e "${gl_bufan}------------------------${gl_bai}"
...
server_reboot() {
    ...
    read -r -e -p "$(echo -e "${gl_huang}提示: ${gl_bai}现在重启服务器吗？(Y/N): ")" rboot
    case "$rboot" in
    [Yy])
        echo "已重启"
        reboot
...
stream_panel() {
    ...
    while true; do
        ...
        2)
            ...
            ;;
        3)
            read -r -e -p "确定要删除 nginx 容器吗？这可能会影响网站功能！(y/N): " confirm
            if [[ "$confirm" =~ ^[Yy]$ ]]; then
                docker rm -f nginx
                sed -i "/\b${app_id}\b/d" /home/docker/appno.txt
                echo "nginx 容器已删除。"
...
ldnmp_web_status() {
    ...
    while true; do
        local cert_count
        cert_count=$(find /home/web/certs/ -name "*_cert.pem" -type f 2>/dev/null | wc -l)
        local output
        output="${gl_lv}${cert_count}${gl_bai}"
        local dbrootpasswd
        dbrootpasswd=$(grep -oP 'MYSQL_ROOT_PASSWORD:\s*\K.*' /home/web/docker-compose.yml | tr -d '[:space:]')
        local db_count
        db_count=$(docker exec mysql mysql -u root -p"$dbrootpasswd" -e "SHOW DATABASES;" 2>/dev/null | grep -Ev "Database|information_schema|mysql|performance_schema|sys" | wc -l)
        local db_output
        db_output="${gl_lv}${db_count}${gl_bai}"
        ...
        1)
            ...
            ;;
        2)
            read -r -e -p "请输入旧域名: " oddyuming
            read -r -e -p "请输入新域名: " yuming
            install_certbot
            install_ssltls
            certs_status
...
linux_trash() {
    ...
    while true; do
        ...
        3)
            ...
            ;;
        4)
            read -r -e -p "确认清空回收站？[y/n]: " confirm
            if [[ "$confirm" == "y" ]]; then
                trash-empty
                echo "回收站已清空。"
            fi
...
format_partition() {
    ...
    *)
    ...
    # 确认格式化
    read -r -e -p "确认格式化分区 /dev/$PARTITION 为 $FS_TYPE 吗？(y/n): " CONFIRM
    if [ "$CONFIRM" != "y" ]; then
        echo "操作已取消。"
        return
    fi
...
linux_info() {
    ...
    # 获取系统信息函数
    get_local_ip() {
        ip -o -4 addr show scope global 2>/dev/null | awk '{print $4}' | cut -d'/' -f1 | head -n1 || echo -e "${gl_bufan}IPv4地址  : ${gl_huang}无法获取${gl_bai}"
    }

    get_cpu_usage() {
        top -bn1 2>/dev/null | grep "Cpu(s)" | awk '{printf "%.2f%%", 100 - $8}' 2>/dev/null || echo -e "${gl_bufan}CPU 占用  : ${gl_huang}无法获取${gl_bai}"
    }
    ...
    get_uptime() {
        ...
        if [ -n "$uptime_seconds" ]; then
            ...
            echo -e "${gl_bufan}运行时间  : ${gl_huang}无法获取${gl_bai}"
        fi
    }
    ...
    get_default_gateway() {
        ...
        if [ -n "$gateway" ]; then
            ...
            echo -e "${gl_bufan}默认网关  : ${gl_huang}无法获取${gl_bai}"
        fi
    }
    ...
    get_disk_usage() {
        ...
        if [ -n "$disk_info" ]; then
            ...
            echo -e "${gl_bufan}磁盘占用  : ${gl_huang}无法获取${gl_bai}"
        fi
    }
    ...
    get_memory_usage() {
        if command -v free >/dev/null 2>&1; then
            free -h 2>/dev/null | awk 'NR==2{printf "%.2fG/%.2fG (%.2f%%)", $3/1024, $2/1024, $3/$2*100}' 2>/dev/null || echo -e "${gl_bufan}内存使用  : ${gl_huang}无法获取${gl_bai}"
        else
            echo -e "${gl_bufan}内存使用  : ${gl_huang}无法获取${gl_bai}"
        fi
    }
    ...
    get_cpu_model() {
        if [ -f /proc/cpuinfo ]; then
            ...
            echo -e "${gl_bufan}CPU 型号  : ${gl_huang}无法获取${gl_bai}"
        fi
    }
    ...
    get_cpu_cores() {
        if [ -f /proc/cpuinfo ]; then
            ...
            if [ -n "$physical_cores" ] && [ -n "$logical_cores" ]; then
                ...
                echo -e "${gl_bufan}CPU 核心  : ${gl_huang}无法获取${gl_bai}"
            fi
        else
            echo -e "${gl_bufan}CPU 核心  : ${gl_huang}无法获取${gl_bai}"
        fi
    ...
    get_system_load() {
        if [ -f /proc/loadavg ]; then
            ...
            if [ -n "$load" ]; then
                ...
                echo -e "${gl_bufan}系统负载  : ${gl_huang}无法获取${gl_bai}"
            fi
        else
            echo -e "${gl_bufan}系统负载  : ${gl_huang}无法获取${gl_bai}"
        fi
    ...
    get_netmask() {
        ...
        if [[ $ip_route =~ /([0-9]+) ]]; then
            ...
            # 备用方法
            ip -o -4 addr show 2>/dev/null | awk '/scope global/ {print $4}' | cut -d'/' -f2 | head -n1 | while read cidr; do
                mask=$((0xffffffff << (32 - cidr) & 0xffffffff))
                echo "$(( (mask >> 24) & 0xff )).$(( (mask >> 16) & 0xff )).$(( (mask >> 8) & 0xff )).$(( mask & 0xff ))"
            done || echo -e "${gl_bufan}网络算法  : ${gl_huang}无法获取${gl_bai}"
        fi
    }

    # 获取所有IPv4地址
    #get_all_ipv4() {
    #    ip -o -4 addr show scope global 2>/dev/null | awk '{print $4}' | cut -d'/' -f1 | tr '\n' ',' | sed 's/,$//' || echo -e "${gl_bufan}xxxx  : ${gl_huang}无法获取${gl_bai}"
    #}

    # 获取DNS服务器
    get_dns_servers() {
        if [ -f /etc/resolv.conf ]; then
            grep -E '^nameserver' /etc/resolv.conf 2>/dev/null | awk '{print $2}' | tr '\n' ',' | sed 's/,$//' || echo -e "${gl_bufan}DNS地址  : ${gl_huang}无法获取${gl_bai}"
        else
            echo -e "${gl_bufan}DNS地址  : ${gl_huang}无法获取${gl_bai}"
        fi
    }

    # 获取网络接口信息
    get_network_interfaces() {
        ip -o link show 2>/dev/null | awk -F': ' '{print $2}' | grep -v lo | head -n3 | tr '\n' ',' | sed 's/,$//' || echo -e "${gl_bufan}网络接口  : ${gl_huang}无法获取${gl_bai}"
    }

    # 获取网络连接状态
    get_network_connections() {
        if command -v netstat >/dev/null 2>&1; then
            netstat -an 2>/dev/null | grep -c ESTABLISHED 2>/dev/null || echo -e "${gl_bufan}连接数    : ${gl_huang}无法获取${gl_bai}"
        elif command -v ss >/dev/null 2>&1; then
            ss -state established 2>/dev/null | grep -c ESTAB 2>/dev/null || echo -e "${gl_bufan}连接数    : ${gl_huang}无法获取${gl_bai}"
        else
            echo -e "${gl_bufan}连接数    : ${gl_huang}无法获取${gl_bai}"
        fi
    }
    ...
    get_process_count() {
        if [ -d /proc ]; then
            ...
            if [ -n "$count" ]; then
                ...
                echo -e "${gl_bufan}进程数量  : ${gl_huang}无法获取${gl_bai}"
            fi
        else
            echo -e "${gl_bufan}进程数量  : ${gl_huang}无法获取${gl_bai}"
        fi
    ...
    get_timezone() {
        if [ -f /etc/timezone ]; then
            cat /etc/timezone 2>/dev/null || echo -e "${gl_bufan}系统时区  : ${gl_huang}未知时区${gl_bai}"
        elif command -v timedatectl >/dev/null 2>&1; then
            timedatectl status 2>/dev/null | grep "Time zone" | awk '{print $3}' || echo -e "${gl_bufan}系统时区  : ${gl_huang}未知时区${gl_bai}"
        else
            date +%Z 2>/dev/null || echo -e "${gl_bufan}系统时区  : ${gl_huang}未知时区${gl_bai}"
        fi
    }

    # 获取当前时间
    get_current_time() {
        date "+%Y-%m-%d %H:%M:%S" 2>/dev/null || echo -e "${gl_bufan}当前时间  : ${gl_huang}无法获取${gl_bai}"
    }

    # 系统信息
    echo -e "${gl_zi}>>> 系统信息${gl_bai}"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    echo -e "${gl_bufan}操作系统  : ${gl_bai}$(get_os_info)"
    ...
    echo -e "${gl_bufan}CPU 核心  : ${gl_bai}$(get_cpu_cores)"
    echo -e "${gl_bufan}CPU 占用  : ${gl_bai}$(get_cpu_usage)"
    echo -e "${gl_bufan}系统负载  : ${gl_bai}$(get_system_load)"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    echo -e "${gl_bufan}内存使用  : ${gl_bai}$(get_memory_usage)"
    echo -e "${gl_bufan}磁盘占用  : ${gl_bai}$(get_disk_usage)"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    echo -e "${gl_bufan}网络接口  : ${gl_bai}$(get_network_interfaces)"
    echo -e "${gl_bufan}IPv4地址  : ${gl_bai}$(get_local_ip)"
    echo -e "${gl_bufan}子网掩码  : ${gl_bai}$(get_netmask)"
    echo -e "${gl_bufan}默认网关  : ${gl_bai}$(get_default_gateway)"
    echo -e "${gl_bufan}DNS地址   : ${gl_bai}$(get_dns_servers)"
    echo -e "${gl_bufan}网络算法  : ${gl_bai}$(get_tcp_congestion)"
    echo -e "${gl_bufan}队列规则  : ${gl_bai}$(get_qdisc)"
    echo -e "${gl_bufan}BBR参数   : ${gl_bai}$(check_bbr_parameters)"
    echo -e "${gl_bufan}连接数    : ${gl_bai}$(get_network_connections)"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    echo -e "${gl_bufan}当前用户  : ${gl_bai}$(get_current_user)"
    echo -e "${gl_bufan}登录用户  : ${gl_bai}$(get_logged_in_users)"
    echo -e "${gl_bufan}进程数量  : ${gl_bai}$(get_process_count)"
    echo -e "${gl_bufan}------------------------${gl_bai}"
    echo -e "${gl_bufan}运行时间  : ${gl_bai}$(get_uptime)"
    echo -e "${gl_bufan}系统时区  : ${gl_bai}$(get_timezone)"
    ...
    restore_docker() {
        ...
        for f in "$BACKUP_DIR"/backup_type_*; do
            ...
            if grep -q "compose" "$f"; then
                ...
                read -r -e -p "确认还原 Compose 项目 [$project_name] 到路径 [$original_path] ? (y/n): " confirm
        ...
        for json in "$BACKUP_DIR"/*_inspect.json; do
            ...
            [[ -z "$IMAGE" || "$IMAGE" == "null" ]] && {
                echo -e "${gl_huang}未找到镜像信息，跳过: $container${NC}"
                continue
            }
...
linux_docker() {
    while true; do
        ...
        6)
	        ...
	        echo -e "${gl_bufan}00. ${gl_bai}退出脚本"
                ...
                2)
                    ...
                    ;;
                3)
                    read -r -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定删除所有未使用的卷吗？(Y/N): ")" choice
                    case "$choice" in
                    [Yy])
        ...
        7)
            clear
            read -r -e -p "$(echo -e "${gl_huang}提示: ${gl_bai}将清理无用的镜像容器网络，包括停止的容器，确定清理吗？(Y/N): ")" choice
            case "$choice" in
            [Yy])
        ...
        20)
            clear
            read -r -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定卸载docker环境吗？(Y/N): ")" choice
            case "$choice" in
            [Yy])
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
        ...
        b)
            ...
            while true; do
                ...
                read -r -e -p "要传送备份数据到远程服务器吗？(Y/N): " choice
                case "$choice" in
                [Yy])
...
linux_Settings() {
    while true; do
        ...
        4)
            ...
            echo -e "${gl_bufan}------------------------${gl_bai}"
            read -r -e -p "输入你要安装的python版本号（输入0退出）: " py_new_v
...
mobufan_update() {
    ...
    while true; do
        ...
        # 获取远程版本号
        local sh_v_new
        sh_v_new=$(curl -s https://gitee.com/meimolihan/script/raw/master/sh/tool/mobufan.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)
        if [ "$sh_v" = "$sh_v_new" ]; then
...
pve_restart_selector() {
    ...
    restart_instance() {
        ...
        read -p "$(echo -e "${gl_bai}确认要${gl_huang}${operation}${gl_bai}这个实例吗? (${gl_lv}y${gl_bai}/${gl_hong}N${gl_bai}): ")" confirm
        case "$confirm" in
        [yY] | [yY][eE][sS])
            ...
            if [ "$instance_status" = "running" ]; then
                ...
                if [ "$instance_type" = "qemu" ]; then
                    ...
                    pct start "$instance_id" && {
                        ...
                        read -p "$(echo -e "${gl_huang}是否尝试强制启动? (y/N): ${gl_bai}")" force
                        [[ $force =~ [yY] ]] && {
    ...
    while true; do
        ...
        3)
            ...
            if ! command -v jq &>/dev/null; then
                ...
                echo
                read -r -e -p "$(echo -e "${gl_huang}是否立即安装 jq? (y/N): ${gl_bai}")" install_choice
...
show_compose_project_menu() {
    ...
    while true; do
        ...
        # 直接使用 read -r 而不是 safe_read，以便更精确地控制行为
    ...
    while true; do
        ...
        88)
            ...
            if [ -d "$repoName" ]; then
                ...
                read -r -rp "$(echo -e "${gl_bufan}是否强制重新克隆? (y/n) ${gl_bai}")" overwrite
                if [[ ${overwrite,,} != "y" ]]; then
    ...
    while true; do
        ...
        1)
            ...
            if ! command -v docker &>/dev/null; then
                ...
                read -r -e -p "$(echo -e "${gl_bufan}是否立即安装 Docker？(y/n): ${gl_bai}")" install_docker
...
remove_rm_redirect() {
    ...
    *)
    ...
    if grep -q "alias rm=" "$config_file" 2>/dev/null; then
        ...
        read -r -e -p "确认移除 rm 命令重定向? [y/N] " -n1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
interactive_delete() {
    ...
    while true; do
        ...
        if [[ -n "$TRASH_CMD" ]]; then
            read -r -e -p "确认移动到回收站? [y/N]： " -n1 -r
        else
            read -r -e -p "确认永久删除? [y/N] " -n1 -r
        fi
        echo
        [[ $REPLY =~ ^[Yy]$ ]] || continue
...
compress_file() {
    ...
    tar.gz | tgz)
        ...
        command -v tar &>/dev/null || {
            echo -e "${gl_hong}错误：tar 命令未安装！${gl_bai}"
            return 1
        }
        ;;
    tar.xz | txz)
        ...
        command -v tar &>/dev/null || command -v xz &>/dev/null || {
            echo -e "${gl_hong}错误：tar 或 xz 命令未安装！${gl_bai}"
            return 1
        }
        ;;
    tar.bz2 | tbz2)
        ...
        command -v tar &>/dev/null || command -v bzip2 &>/dev/null || {
            echo -e "${gl_hong}错误：tar 或 bzip2 命令未安装！${gl_bai}"
            return 1
        }
        ;;
    tar)
...
docker_image_pack() {
    ...
    while true; do
        ...
        [ "$IMG" = "0" ] && { set +euo pipefail; return 0; }   # 直接返回
        [ -z "$IMG" ]   && { echo -e "${gl_hong}输入不能为空！${gl_bai}"; continue; }
    ...
    while true; do
        ...
        [ "$FMT" = "0" ] && { set +euo pipefail; return 0; }
        case "$FMT" in 1) EXT="tar"; break;; 2) EXT="tar.gz"; break;; *) echo -e "${gl_hong}输入错误！${gl_bai}";; esac
    done

    # ---------- 4. 获取文件名 ----------
    while true; do
        read -r -erp "$(echo -e "${gl_bufan}请输入打包文件名（${gl_huang}不含扩展名${gl_bai}，输入 ${gl_huang}0${gl_bai} 返回）：") " FNAME
        [ "$FNAME" = "0" ] && { set +euo pipefail; return 0; }
        [ -z "$FNAME" ] && { echo -e "${gl_hong}文件名不能为空！${gl_bai}"; continue; }
        [[ "$FNAME" =~ [^a-zA-Z0-9._-] ]] && { echo -e "${gl_hong}文件名含非法字符！${gl_bai}"; continue; }
        [ -f "${FNAME}.${EXT}" ] && {
            read -r -erp "$(echo -e "${gl_huang}文件已存在，是否覆盖？${gl_bai} (y/N): ")" OVER
...
mobufan_sh() {
    while true; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}9.   ${gl_bai}PVE命令"
