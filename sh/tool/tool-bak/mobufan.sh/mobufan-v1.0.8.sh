...
while true; do
	...
	case "$choice" in
		1)
			...
			CheckFirstRun_true
			yinsiyuanquan2
			cp -f ~/mobufan.sh /usr/local/bin/k > /dev/null 2>&1
...
linux_pve_menu() {
	...
	while true; do
	  ...
	  echo -e "PVE 系统命令"
          echo -e "${gl_bufan}------------------------"
          echo -e "${gl_bufan}当前工作目录: $(pwd)"
          echo -e "${gl_bufan}内网IP: $(get_internal_ip)"
          echo -e "${gl_bufan}------------------------"
...
show_compose_project_menu() {
    ...
    while true; do
        ...
        echo "========================================"
        echo -e "${gl_bufan}当前工作目录: $(pwd)"
        echo -e "${gl_bufan}基础路径: $base_path"
        echo -e "${gl_bufan}内网IP: $(get_internal_ip)"
        echo "----------------------------------------"
        echo -e "${gl_huang}请选择要进入的项目："
        ...
        for project in "${projects[@]}"; do
            count=$((count + 1))
            printf "${gl_bufan}%2d. 进入 %-30s${NC}" "$count" "$project"
            # 每行显示两个项目
...
show_compose_commands_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}1. 启动服务 (docker-compose up -d)"
        echo -e "${gl_bufan}2. 停止服务 (docker-compose down)"
        echo -e "${gl_bufan}3. 重启服务 (docker-compose restart)"
        echo -e "${gl_bufan}4. 查看服务状态 (docker-compose ps)"
        echo -e "${gl_bufan}5. 查看服务日志 (docker-compose logs)"
        echo -e "${gl_bufan}6. 重新构建并启动 (docker-compose up -d --build)"
	echo -e "${gl_bufan}------------------------"
        echo -e "${gl_bufan}0. 返回项目列表"
        echo "========================================"
        ...
        case $cmd_choice in
            1)
                echo -e "${gl_bufan}执行命令: docker-compose up -d"
                echo "----------------------------------------"
                docker-compose up -d
                echo "----------------------------------------"
                ;;
            2)
                echo -e "${gl_bufan}执行命令: docker-compose down"
                echo "----------------------------------------"
                docker-compose down
                echo "----------------------------------------"
                ;;
            3)
                echo -e "${gl_bufan}执行命令: docker-compose restart"
                echo "----------------------------------------"
                docker-compose restart
                echo "----------------------------------------"
                ;;
            4)
                echo -e "${gl_bufan}执行命令: docker-compose ps"
                echo "----------------------------------------"
                docker-compose ps
                echo "----------------------------------------"
                ;;
            5)
                echo -e "${gl_bufan}执行命令: docker-compose logs"
                echo "----------------------------------------"
                docker-compose logs
                echo "----------------------------------------"
                ;;
            6)
                echo -e "${gl_bufan}执行命令: docker-compose up -d --build"
                echo "----------------------------------------"
...
linux_fnos_menu() {
	...
	while true; do
	  clear
	  # send_stats "FnOS 系统命令"
	  echo -e "FnOS 系统命令"
          echo -e "${gl_bufan}------------------------"
          echo -e "${gl_bufan}当前工作目录: $(pwd)"
          echo -e "${gl_bufan}内网IP: $(get_internal_ip)"
          echo -e "${gl_bufan}------------------------"
          echo -e "${gl_bufan}1.   ${gl_bai}Compose容器管理"
          echo -e "${gl_bufan}------------------------"
...
clear
echo -e "${gl_bufan}"
echo -e "mobufan脚本工具箱 v$sh_v"
echo -e "命令行输入${gl_huang}g${gl_bufan}可快速启动脚本${gl_bai}"
echo -e "${gl_bufan}------------------------${gl_bai}"
...
echo -e "${gl_huang}8.   ${gl_bai}系统工具"
echo -e "${gl_bufan}9.   ${gl_bai}PVE系统命令"
echo -e "${gl_bufan}10.  ${gl_bai}FnOS系统命令"
echo -e "${gl_bufan}------------------------${gl_bai}"
