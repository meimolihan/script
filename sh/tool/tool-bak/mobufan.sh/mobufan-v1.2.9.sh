...
# 函数：安装 Git（自动检测 Linux 发行版）
install_git() {
    # 检查是否已安装 Git
    if command -v git &> /dev/null; then
        echo -e "${gl_lv}Git 已安装，版本: $(git --version)${gl_bai}"
        return 0
    fi

    echo -e "${gl_huang}检测到系统未安装 Git，正在自动安装...${gl_bai}"
    ...
    if [ -f /etc/os-release ]; then
        ...
        case $ID in
            ubuntu|debian|linuxmint)
                echo -e "${gl_huang}检测到 Debian 系发行版 ($PRETTY_NAME)，使用 apt 安装 Git${gl_bai}"
                if sudo apt update && sudo apt install -y git; then
                    echo -e "${gl_lv}Git 安装成功！版本: $(git --version)${gl_bai}"
                    return 0
                else
                    echo -e "${gl_hong}Git 安装失败${gl_bai}"
                    return 1
                fi
                ;;
            centos|rhel|fedora|rocky|almalinux)
                echo -e "${gl_huang}检测到 Red Hat 系发行版 ($PRETTY_NAME)，使用 yum/dnf 安装 Git${gl_bai}"
                ...
                ;;
            arch|manjaro)
                echo -e "${gl_huang}检测到 Arch 系发行版 ($PRETTY_NAME)，使用 pacman 安装 Git${gl_bai}"
                ...
                ;;
            opensuse*|suse)
                echo -e "${gl_huang}检测到 openSUSE/SUSE 发行版 ($PRETTY_NAME)，使用 zypper 安装 Git${gl_bai}"
                if sudo zypper refresh && sudo zypper install -y git; then
                    echo -e "${gl_lv}Git 安装成功！版本: $(git --version)${gl_bai}"
                    return 0
                else
                    echo -e "${gl_hong}Git 安装失败${gl_bai}"
                ...
                ;;
            alpine)
                echo -e "${gl_huang}检测到 Alpine Linux ($PRETTY_NAME)，使用 apk 安装 Git${gl_bai}"
                if sudo apk update && sudo apk add git; then
                    echo -e "${gl_lv}Git 安装成功！版本: $(git --version)${gl_bai}"
                    return 0
                else
                    echo -e "${gl_hong}Git 安装失败${gl_bai}"
                ...
                ;;
            *)
                echo -e "${gl_hong}无法识别的 Linux 发行版: $ID${gl_bai}"
                echo -e "${gl_huang}请手动安装 Git:${gl_bai}"
                echo -e "${gl_huang}- Debian/Ubuntu: sudo apt install git${gl_bai}"
                echo -e "${gl_huang}- CentOS/RHEL: sudo yum install git${gl_bai}"
                echo -e "${gl_huang}- Fedora: sudo dnf install git${gl_bai}"
                echo -e "${gl_huang}- Arch: sudo pacman -S git${gl_bai}"
                echo -e "${gl_huang}- openSUSE: sudo zypper install git${gl_bai}"
                return 1
                ;;
        esac
    else
        echo -e "${gl_hong}无法检测 Linux 发行版，请手动安装 Git${gl_bai}"
        return 1
    fi
}
...
safe_read() {
    local prompt="$1"
    local variable="$2"
    local validation="${3:-any}"  # 验证类型：number, file, dir, any（默认）
...
safe_read_multiple_ids() {
    ...
    while true; do
        ...
        for id in "${ids[@]}"; do
            ...
            # 数字范围检查
            if [ -n "$min" ] && [ "$id" -lt "$min" ]; then
                echo "错误: 数字 '$id' 不能小于 $min"
                valid=false
                break
            fi
            
            if [ -n "$max" ] && [ "$id" -gt "$max" ]; then
                echo "错误: 数字 '$id' 不能大于 $max"
                valid=false
                break
            fi
        done
...
sed -i '/^alias g=/d' ~/.bashrc > /dev/null 2>&1
sed -i '/^alias g=/d' ~/.profile > /dev/null 2>&1
sed -i '/^alias g=/d' ~/.bash_profile > /dev/null 2>&1
...
UserLicenseAgreement() {
	clear
	echo -e "${gl_bufan}欢迎使用mobufan脚本工具箱${gl_bai}"
...
while true; do
	...
	echo -e "${gl_bufan}------------------------${gl_bai}"
	echo -e "${gl_huang}1. ${gl_bai}创建新的容器"
	echo -e "${gl_bufan}------------------------${gl_bai}"
	echo -e "${gl_huang}2. ${gl_bai}启动指定容器             ${gl_huang}6. ${gl_bai}启动所有容器"
	echo -e "${gl_huang}3. ${gl_bai}停止指定容器             ${gl_huang}7. ${gl_bai}停止所有容器"
	echo -e "${gl_huang}4. ${gl_bai}删除指定容器             ${gl_huang}8. ${gl_bai}删除所有容器"
	echo -e "${gl_huang}5. ${gl_bai}重启指定容器             ${gl_huang}9. ${gl_bai}重启所有容器"
	echo -e "${gl_bufan}------------------------${gl_bai}"
	echo -e "${gl_huang}11. ${gl_bai}进入指定容器           ${gl_huang}12. ${gl_bai}查看容器日志"
	echo -e "${gl_huang}13. ${gl_bai}查看容器网络           ${gl_huang}14. ${gl_bai}查看容器占用"
	echo -e "${gl_bufan}------------------------${gl_bai}"
	echo -e "${gl_huang}15. ${gl_bai}开启容器端口访问       ${gl_huang}16. ${gl_bai}关闭容器端口访问"
	echo -e "${gl_bufan}------------------------${gl_bai}"
...
while true; do
	...
	echo ""
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}1. ${gl_bai}获取指定镜像             ${gl_huang}3. ${gl_bai}删除指定镜像"
        echo -e "${gl_huang}2. ${gl_bai}更新指定镜像             ${gl_huang}4. ${gl_bai}删除所有镜像"
        echo -e "${gl_huang}5. ${gl_bai}打包指定镜像             ${gl_huang}6. ${gl_bai}加载指定镜像"
        echo -e "${gl_huang}7. ${gl_bai}为镜像打标签             "
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}0. ${gl_bai}返回上一级选单"
        echo -e "${gl_bufan}------------------------${gl_bai}"
	read -e -p "请输入你的选择: " sub_choice
		...
		6)
			...
			;;
                ...
                7)
                    ...
                    # 输入源镜像
                    read -p "请输入源镜像名称 (格式: name[:tag] 或 image_id): " source_image
                    ...
                    # 输入目标镜像信息
                    read -p "请输入目标镜像名称 (不含标签): " target_name
                    read -p "请输入目标镜像标签 [默认: latest]: " target_tag
...
linux_info() {
	...
	echo -e "系统信息查询"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}主机名:       ${gl_bai}$hostname"
	echo -e "${gl_bufan}系统版本:     ${gl_bai}$os_info"
	echo -e "${gl_bufan}Linux版本:    ${gl_bai}$kernel_version"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}CPU架构:      ${gl_bai}$cpu_arch"
	echo -e "${gl_bufan}CPU型号:      ${gl_bai}$cpu_info"
	echo -e "${gl_bufan}CPU核心数:    ${gl_bai}$cpu_cores"
	echo -e "${gl_bufan}CPU频率:      ${gl_bai}$cpu_freq"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}CPU占用:      ${gl_bai}$cpu_usage_percent%"
	echo -e "${gl_bufan}系统负载:     ${gl_bai}$load"
	echo -e "${gl_bufan}物理内存:     ${gl_bai}$mem_info"
	echo -e "${gl_bufan}虚拟内存:     ${gl_bai}$swap_info"
	echo -e "${gl_bufan}硬盘占用:     ${gl_bai}$disk_info"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}总接收:       ${gl_bai}$rx"
	echo -e "${gl_bufan}总发送:       ${gl_bai}$tx"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}网络算法:     ${gl_bai}$congestion_algorithm $queue_algorithm"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}运营商:       ${gl_bai}$isp_info"
	if [ -n "$ipv4_address" ]; then
		echo -e "${gl_bufan}IPv4地址:     ${gl_bai}$ipv4_address"
	fi

	if [ -n "$ipv6_address" ]; then
		echo -e "${gl_bufan}IPv6地址:     ${gl_bai}$ipv6_address"
	fi
	echo -e "${gl_bufan}DNS地址:      ${gl_bai}$dns_addresses"
	echo -e "${gl_bufan}地理位置:     ${gl_bai}$country $city"
	echo -e "${gl_bufan}系统时间:     ${gl_bai}$timezone $current_time"
	echo -e "${gl_bufan}-------------"
	echo -e "${gl_bufan}运行时长:     ${gl_bai}$runtime"
	echo
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "基础工具"
	  echo -e "${gl_bufan}------------------------"
	  echo -e "${gl_bufan}1.   ${gl_bai}curl 下载工具 ${gl_huang}★${gl_bai}                   ${gl_bufan}2.   ${gl_bai}wget 下载工具 ${gl_huang}★${gl_bai}"
	  echo -e "${gl_bufan}3.   ${gl_bai}sudo 超级管理权限工具             ${gl_bufan}4.   ${gl_bai}socat 通信连接工具"
	  echo -e "${gl_bufan}5.   ${gl_bai}htop 系统监控工具                 ${gl_bufan}6.   ${gl_bai}iftop 网络流量监控工具"
	  echo -e "${gl_bufan}7.   ${gl_bai}unzip ZIP压缩解压工具             ${gl_bufan}8.   ${gl_bai}tar GZ压缩解压工具"
	  echo -e "${gl_bufan}9.   ${gl_bai}tmux 多路后台运行工具             ${gl_bufan}10.  ${gl_bai}ffmpeg 视频编码直播推流工具"
	  echo -e "${gl_bufan}------------------------"
	  echo -e "${gl_bufan}11.  ${gl_bai}btop 现代化监控工具 ${gl_huang}★${gl_bai}             ${gl_bufan}12.  ${gl_bai}ranger 文件管理工具"
	  echo -e "${gl_bufan}13.  ${gl_bai}ncdu 磁盘占用查看工具             ${gl_bufan}14.  ${gl_bai}fzf 全局搜索工具"
	  echo -e "${gl_bufan}15.  ${gl_bai}vim 文本编辑器                    ${gl_bufan}16.  ${gl_bai}nano 文本编辑器 ${gl_huang}★${gl_bai}"
	  echo -e "${gl_bufan}17.  ${gl_bai}git 版本控制系统"
	  echo -e "${gl_bufan}------------------------"
...
docker_ssh_migration() {
	...
	backup_docker() {
		...
		echo -e "${gl_bufan}正在备份 Docker 容器...${NC}"
		...
		[[ ${#TARGET_CONTAINERS[@]} -eq 0 ]] && { echo -e "${gl_huang}没有找到容器${NC}"; return; }
		...
		for c in "${TARGET_CONTAINERS[@]}"; do
			...
			if is_compose_container "$c"; then
				...
				if [[ -n "${PACKED_COMPOSE_PATHS[$project_dir]}" ]]; then
					echo -e "${gl_bufan}Compose 项目 [$project_name] 已备份过，跳过重复打包...${NC}"
					continue
				fi
				...
				if [ -f "$project_dir/docker-compose.yml" ]; then
					...
					echo -e "${gl_huang}未找到 docker-compose.yml，跳过此容器...${NC}"
				fi
			else
				# 普通容器备份卷
				local VOL_PATHS
	...
	restore_docker() {
		...
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${gl_huang}备份目录不存在${NC}"; return; }
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				if [[ "$running_count" -gt 0 ]]; then
					echo -e "${gl_bufan}Compose 项目 [$project_name] 已有容器在运行，跳过还原...${NC}"
					continue
				fi
		...
		for json in "$BACKUP_DIR"/*_inspect.json; do
			...
			if docker ps --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${gl_bufan}容器 [$container] 已在运行，跳过还原...${NC}"
				continue
			fi

			IMAGE=$(jq -r '.[0].Config.Image' "$json")
			[[ -z "$IMAGE" || "$IMAGE" == "null" ]] && { echo -e "${gl_huang}未找到镜像信息，跳过: $container${NC}"; continue; }
			...
			if docker ps -a --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${gl_bufan}容器 [$container] 存在但未运行，删除旧容器...${NC}"
		...
		[[ "$has_container" == false ]] && echo -e "${gl_bufan}未找到普通容器的备份信息${NC}"
		...
		if [ -f "$BACKUP_DIR/home_docker_files.tar.gz" ]; then
			...
			echo -e "${gl_bufan}未找到 /home/docker 下文件的备份，跳过...${NC}"
		fi
		  ...
		  5)
			  while true; do
				  ...
				  echo ""
                                  ...
                                  echo -e "${gl_bufan}------------------------${gl_bai}"
                                  echo -e "${gl_huang}1. ${gl_bufan}创建网络"
                                  echo -e "${gl_huang}2. ${gl_bufan}加入网络"
                                  echo -e "${gl_huang}3. ${gl_bufan}退出网络"
                                  echo -e "${gl_huang}4. ${gl_bufan}删除网络"
                                  echo -e "${gl_bufan}------------------------${gl_bai}"
                                  echo -e "${gl_huang}0. ${gl_bufan}返回上一级选单"
                                  echo -e "${gl_bufan}------------------------${gl_bai}"
				  read -e -p "请输入你的选择: " sub_choice
		  ...
		  6)
			  while true; do
				  ...
				  echo ""
                                  ...
                                  echo -e "${gl_bufan}------------------------${gl_bai}"
                                  echo -e "${gl_huang}1. ${gl_bai}创建新卷"
                                  echo -e "${gl_huang}2. ${gl_bai}删除指定卷"
                                  echo -e "${gl_huang}3. ${gl_bai}删除所有卷"
                                  echo -e "${gl_bufan}------------------------${gl_bai}"
                                  echo -e "${gl_huang}0. ${gl_bai}返回上一级选单"
                                  echo -e "${gl_bufan}------------------------${gl_bai}"
				  read -e -p "请输入你的选择: " sub_choice
...
while true; do
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_bufan}5.   ${color5}OpenList多存储文件列表程序          ${gl_bufan}6.   ${color6}Ubuntu远程桌面网页版"
	  ...
	  echo -e "${gl_bufan}43.  ${color43}RustDesk远程桌面(服务端) ${gl_huang}★${gl_bai}          ${gl_bufan}44.  ${color44}RustDesk远程桌面(中继端) ${gl_huang}★${gl_bai}"
	  ...
	  echo -e "${gl_bufan}67.  ${color67}ddns-go动态DNS管理工具 ${gl_huang}★${gl_bai}            ${gl_bufan}68.  ${color68}AllinSSL证书管理平台"
...
linux_Settings() {
	while true; do
	  ...
	  echo -e "${gl_bufan}9.   ${gl_bai}禁用ROOT账户创建新账户             ${gl_bufan}10.  ${gl_bai}切换优先ipv4/ipv6"
	  echo -e "${gl_bufan}------------------------"
	  ...
	  echo -e "${gl_bufan}27.  ${gl_bai}红帽系Linux内核升级                ${gl_bufan}28.  ${gl_bai}Linux系统内核参数优化 ${gl_huang}★${gl_bai}"
...
create_file() {
    while true; do
        ...
        echo -e "${gl_bufan}请输入文件内容(输入单行 'EOF' 结束输入，支持退格键修改):${gl_bai}"
        ...
        # 读取多行输入（增强版，支持更好的编辑）
        line_count=0
        echo -e "${gl_bufan}开始输入内容 (第 $((line_count+1)) 行):${gl_bai}"
        ...
        while true; do
            ...
            echo -e "${gl_bufan}已输入 $line_count 行，继续输入 (输入 'EOF' 结束):${gl_bai}"
        done
        
        if [ $line_count -eq 0 ]; then
            echo -e "${gl_bufan}未输入内容，创建空文件${gl_bai}"
            touch "$file_name"
        else
            # 将内容写入目标文件
            mv "$temp_file" "$file_name"
            echo -e "${gl_huang}文件 '$file_name' 创建成功，共写入 $line_count 行${gl_bai}"
        fi
        
        # 显示文件内容预览
        echo -e "${gl_bufan}文件内容预览:${gl_bai}"
        echo "----------------------------------------"
        if [ -s "$file_name" ]; then
            cat -n "$file_name" 2>/dev/null || echo -e "${gl_hong}无法显示文件内容${gl_bai}"
        else
            echo -e "${gl_huang}(空文件)${gl_bai}"
        fi
        echo "----------------------------------------"
        ...
        if [[ "$file_name" =~ \.sh$ ]] || [[ "$file_name" =~ \.py$ ]] || [[ "$file_name" =~ \.pl$ ]]; then
            ...
            if [ "$add_exec" = "y" ]; then
                chmod +x "$file_name"
                echo -e "${gl_huang}已为文件 '$file_name' 添加执行权限${gl_bai}"
            fi
        fi
...
linux_file() {
	...
	while true; do
		...
		ls --color=auto -x
		echo "------------------------"
		echo -e "${gl_huang}1.  ${gl_bufan}进入目录        ${gl_huang}2.  ${gl_bufan}创建目录          ${gl_huang}3.  ${gl_bufan}修改目录权限      ${gl_huang}4.  ${gl_bufan}重命名目录${gl_bai}"
		echo -e "${gl_huang}5.  ${gl_bufan}删除目录        ${gl_huang}6.  ${gl_bufan}返回上一级目录"
		echo "------------------------"
		echo -e "${gl_huang}11. ${gl_bufan}创建文件        ${gl_huang}12. ${gl_bufan}编辑文件          ${gl_huang}13. ${gl_bufan}创建并编辑文件    ${gl_huang}14. ${gl_bufan}查看文件内容${gl_bai}"
		echo -e "${gl_huang}15. ${gl_bufan}删除文件        ${gl_huang}16. ${gl_bufan}重命名文件        ${gl_huang}17. ${gl_bufan}修改文件权限"
		echo "------------------------"
		echo -e "${gl_huang}21. ${gl_bufan}压缩文件目录    ${gl_huang}22. ${gl_bufan}解压文件目录      ${gl_huang}23. ${gl_bufan}解压压缩工具      ${gl_huang}24. ${gl_bufan}移动文件目录${gl_bai}"
		echo -e "${gl_huang}25. ${gl_bufan}复制文件目录    ${gl_huang}26. ${gl_bufan}传文件至其他服务器"
		echo "------------------------"
		echo -e "${gl_huang}99. ${gl_bufan}文件回收站${gl_bai}"
		echo "------------------------"
		echo -e "${gl_huang}0.  ${gl_bufan}返回上一级选单${gl_bai}"
		echo "------------------------"
		read -e -p "请输入你的选择: " Limiting
		...
		case "$Limiting" in
			...
			4)  # 重命名目录
				...
				;;
                        5)  # 删除目录
                            read -e -p "请输入要删除的目录名（多个目录请用空格分隔）: " dirnames
			...
			13) # 创建并编辑文件
				...
				;;
			14) # 查看文件内容
				read -e -p "请输入要查看的文件名: " filename
				cat "$filename"
                                break_end
				send_stats "查看文件内容"
				;;
                        15) # 删除文件
                            read -e -p "请输入要删除的文件名（多个文件请用空格分隔）: " filenames
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: https://gitee.com/meimolihan/script/raw/master/sh/tool/mobufan_sh_log.txt"
	echo "------------------------"

	curl -s https://gitee.com/meimolihan/script/raw/master/sh/tool/mobufan_sh_log.txt | tail -n 30
	local sh_v_new=$(curl -s https://gitee.com/meimolihan/script/raw/master/sh/tool/mobufan.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)
	...
	if [ -n "$existing_cron" ]; then
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}1. ${gl_bai}现在更新               ${gl_huang}2. ${gl_bai}强制更新"
        echo -e "${gl_huang}3. ${gl_bai}开启自动更新           ${gl_huang}4. ${gl_bai}关闭自动更新"
        echo -e "${gl_bufan}------------------------${gl_bai}"
	...
	case "$choice" in
		1)
			...
			local country=$(curl -s ipinfo.io/country)
			if [ "$country" = "CN" ]; then
				curl -sS -O https://gitee.com/meimolihan/script/raw/master/sh/install/mobufan.sh && chmod +x mobufan.sh
			else
				curl -sS -O https://script.meimolihan.eu.org/sh/install/mobufan.sh && chmod +x mobufan.sh
			fi
			canshu_v6
			CheckFirstRun_true
			yinsiyuanquan2
		...
		3)
			...
			if [ "$country" = "CN" ]; then
				SH_Update_task="curl -sS -O https://gitee.com/meimolihan/script/raw/master/sh/install/mobufan.sh && chmod +x mobufan.sh && sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ./mobufan.sh"
			elif [ -n "$ipv6_address" ]; then
				SH_Update_task="curl -sS -O https://gitee.com/meimolihan/script/raw/master/sh/install/mobufan.sh && chmod +x mobufan.sh && sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ./mobufan.sh"
			else
				SH_Update_task="curl -sS -O https://gitee.com/meimolihan/script/raw/master/sh/install/mobufan.sh && chmod +x mobufan.sh"
			fi
			check_crontab_installed
			(crontab -l | grep -v "mobufan.sh") | crontab -
...
show_running_instances() {
    ...
    shutdown_instance() {
        ...
        read -p "$(echo -e "${gl_huang}确认要关闭这个实例吗? (y/N): ${gl_bai}")" confirm
        case "$confirm" in
            [yY]|[yY][eE][sS])
                ...
                if [ "$instance_type" = "qemu" ]; then
                    qm shutdown "$instance_id" &&
                        ...
                        { echo -e "${gl_hong}关闭虚拟机 $instance_id 失败${gl_bai}"; return 1; }
                else
                    pct shutdown "$instance_id" &&
                        echo -e "${gl_lv}LXC容器 $instance_id 关闭命令已发送${gl_bai}" ||
                        { echo -e "${gl_hong}关闭LXC容器 $instance_id 失败${gl_bai}"; return 1; }
                fi
                ;;
            *) echo -e "${gl_huang}操作已取消${gl_bai}"; return 2 ;;
        esac
        return 0
    }
    ...
    while :; do
        read -e -p "$(echo -e "${gl_bufan}请输入要${gl_huang}关闭${gl_bai}的实例序号 (输入 ${gl_huang}0${gl_bai} 返回): ")" choice
       case $choice in
            0)
                return 0        # 用 return 直接跳出函数
                ;;
            ''|*[!0-9]*)
            ...
            *)
                ...
                if [ "$index" -ge 0 ] && [ "$index" -lt "${#RUNNING_INSTANCES[@]}" ]; then
                    ...
                    if [ $? -eq 0 ]; then
                        ...
                        printf "\r\033[K"   # 清掉倒计时行
                        read -t 0.001 -n 1000 2>/dev/null  # 清空 stdin 残留
                        clear
...
pve_start_selector() {
    ...
    get_stopped_instances() {
        ...
        INSTANCE_NAMES=()
        clear
        echo -e "${gl_zi}>>> 正在获取已停止的实例...${gl_bai}"
    ...
    start_instance() {
        ...
        read -p "$(echo -e "${gl_huang}确认要启动这个实例吗? (y/N): ${gl_bai}")" confirm
        case "$confirm" in
            [yY]|[yY][eE][sS])
                ...
                if [ "$instance_type" = "qemu" ]; then
                    qm start "$instance_id" &&
                        ...
                        { echo -e "${gl_hong}启动虚拟机 $instance_id 失败${gl_bai}"; return 1; }
                else
                    pct start "$instance_id" &&
                        echo -e "${gl_lv}LXC容器 $instance_id 启动命令已发送${gl_bai}" ||
                        { echo -e "${gl_hong}启动LXC容器 $instance_id 失败${gl_bai}"; return 1; }
                fi
                ;;
            *) echo -e "${gl_huang}操作已取消${gl_bai}"; return 2 ;;
        esac
    ...
    while :; do
        read -e -p "$(echo -e "${gl_bufan}请输入要${gl_huang}启动${gl_bai}的实例序号 (输入 ${gl_huang}0${gl_bai} 返回): ")" choice
       case $choice in
            0)
                return 0        # 用 return 直接跳出函数
                ;;
...
pve_restart_selector() {
    ...
    get_all_instances() {
        clear
        echo -e "${gl_zi}>>> 正在获取所有实例信息...${gl_bai}"
        ...
        while IFS= read -r line; do
            if [ -n "$line" ]; then
                local vmid=$(echo "$line" | jq -r '.vmid')
	...
	echo -e "${gl_bufan}------------------------${gl_bai}"
        ...
        # 计算每个实例显示的最大宽度（不考虑颜色代码）
        local max_width=0
        for ((i=0; i<total_instances; i++)); do
            local type_str
            if [ "${INSTANCE_TYPES[$i]}" = "qemu" ]; then
                type_str="VM"
            else
                type_str="LXC"
            fi
            local display_text="${type_str}:${INSTANCE_IDS[$i]}"
            local text_length=${#display_text}
            if [ $text_length -gt $max_width ]; then
                max_width=$text_length
            fi
        done
        max_width=$((max_width + 4)) # 增加一些间距
        
        for ((i=0; i<total_instances; i+=instances_per_line)); do
            for ((j=0; j<instances_per_line; j++)); do
                local index=$((i+j))
                if [ $index -lt $total_instances ]; then
                    local type_str
                    local status_color
                    local status_symbol
                    
                    if [ "${INSTANCE_TYPES[$index]}" = "qemu" ]; then
                        type_str="VM"
                    else
                        type_str="LXC"
                    fi
                    ...
                    local current_length=${#current_text}
                    local padding=$((max_width - current_length))
                    printf "%*s" $padding ""
                fi
            done
            echo  # 换行
    ...
    show_instance_details() {
        ...
        echo -e "  ${gl_lan}名称:${gl_bai} ${gl_huang}$instance_name${gl_bai}"
        echo -e "  ${gl_lan}状态:${gl_bai} ${status_color}$status_str${gl_bai}"
    }
    ...
    restart_instance() {
        ...
        # 确认操作（只提示一次）
        echo
        read -p "$(echo -e "${gl_huang}确认要${operation}这个实例吗? (y/N): ${gl_bai}")" confirm
        case "$confirm" in
            [yY]|[yY][eE][sS])
                ...
                if [ "$instance_status" = "running" ]; then
                    ...
                    if [ "$instance_type" = "qemu" ]; then
                        ...
                        # 重启LXC容器
                        if pct reboot "$instance_id"; then
                            echo -e "${gl_lv}LXC容器 $instance_id 重启命令已发送${gl_bai}"
                        else
                            echo -e "${gl_hong}重启LXC容器 $instance_id 失败，尝试使用停止/启动方式${gl_bai}"
                            # 如果reboot失败，尝试stop/start
                            if pct shutdown "$instance_id" && pct start "$instance_id"; then
                    ...
                    if [ "$instance_type" = "qemu" ]; then
                        ...
                        # 启动LXC容器
                        if pct start "$instance_id"; then
                            echo -e "${gl_lv}LXC容器 $instance_id 启动命令已发送${gl_bai}"
                        else
                            echo -e "${gl_hong}启动LXC容器 $instance_id 失败${gl_bai}"
                            # 尝试强制启动
                            read -p "$(echo -e "${gl_huang}是否尝试强制启动? (y/N): ${gl_bai}")" force_confirm
                            case "$force_confirm" in
                                [yY]|[yY][eE][sS])
                                    if pct start "$instance_id" --force; then
                                        echo -e "${gl_lv}LXC容器 $instance_id 强制启动成功${gl_bai}"
                                    else
                                        echo -e "${gl_hong}强制启动也失败了，请检查容器配置${gl_bai}"
                                        return 1
                                    fi
                                    ;;
                                *)
    ...
    # 主逻辑
    if ! get_all_instances; then
        return 1
    fi
    
    if ! show_all_instances; then
        return 1
    fi
    ...
    while true; do
        echo
        echo -e "${gl_bufan}------------------------${gl_bai}"
        read -e -p "$(echo -e "${gl_bufan}请输入要操作的实例序号 (输入 ${gl_huang}0${gl_bai} 退出): ")" choice
        case "$choice" in
            0)
            ...
            *)
                ...
                if [[ "$choice" =~ ^[0-9]+$ ]]; then
                    ...
                    if [ "$index" -ge 0 ] && [ "$index" -lt "${#ALL_INSTANCES[@]}" ]; then
                        ...
                        # 操作完成后暂停一下，然后重新显示列表
                        echo
                        read -p "$(echo -e "${gl_huang}按任意键返回实例列表...${gl_bai}")" -n 1 -s
                        clear
                        echo
                        # 重新获取实例列表以更新状态
                        if ! get_all_instances; then
                        ...
                        echo -e "${gl_hong}错误: 序号 ${choice} 不在有效范围内${gl_bai}"
                        read -p "$(echo -e "${gl_huang}按任意键继续...${gl_bai}")" -n 1 -s
                        echo
                    fi
                else
                    echo -e "${gl_hong}错误: 请输入有效的数字序号${gl_bai}"
...
pve_instance_management() {
    while true; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}1.  ${gl_bufan} 启动虚拟机${gl_bai}"
        echo -e "${gl_huang}2.  ${gl_bufan} 关闭虚拟机${gl_bai}" 
        echo -e "${gl_huang}3.  ${gl_bufan} 启动LXC容器${gl_bai}"
        echo -e "${gl_huang}4.  ${gl_bufan} 关闭LXC容器${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}0.  ${gl_bufan} 返回主菜单${gl_bai} "
        echo -e "${gl_bufan}------------------------${gl_bai}"
        read -e -p "请输入你的选择: " choice
...
linux_pve_menu() {
	while true; do
	  ...
	  echo -e "PVE命令"
          ...
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_huang}1.  ${gl_bufan}查看所有虚拟机${gl_bai}"
          echo -e "${gl_huang}2.  ${gl_bufan}PVE手动管理实例${gl_bai}"
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_huang}3.  ${gl_bufan}交互式关闭虚拟机${gl_bai}"
          echo -e "${gl_huang}4.  ${gl_bufan}交互式开启虚拟机${gl_bai}"
          echo -e "${gl_huang}5.  ${gl_bufan}交互式重启虚拟机${gl_bai}"
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_huang}6.  ${gl_bufan}管理备份目录${gl_bai}"
          echo -e "${gl_huang}7.  ${gl_bufan}管理固件目录${gl_bai}"
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_huang}8.  ${gl_bufan}PVE 更新并清理系统${gl_bai}"
          echo -e "${gl_huang}9.  ${gl_bufan}PVE 优化脚本${gl_bai}"
          echo -e "${gl_bufan}------------------------${gl_bai}"
	  echo -e "${gl_huang}0.  ${gl_bufan}返回主菜单${gl_bai}"
	  echo -e "${gl_bufan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
	  case $sub_choice in
		  1)
			...
			;;
                  2)
                      # PVE手动管理实例
                       send_stats "PVE手动管理实例"
                       if [ ! -d "/var/lib/vz/template/iso" ]; then
                            echo -e "${gl_bufan}错误：这不是PVE系统！${gl_bai}"
                            read -p "按任意键返回主菜单..."
                            continue
                        fi
			 clear
                    ...
                    3)
                        ...
                        if ! command -v jq &> /dev/null; then
                            ...
                            echo
                            read -p "$(echo -e "${gl_huang}是否立即安装 jq? (y/N): ${gl_bai}")" install_choice
...
show_compose_project_menu() {
    local base_path="$(pwd)"  # 使用当前目录作为基础路径
    ...
    while true; do
        ...
        # 直接使用 read 而不是 safe_read，以便更精确地控制行为
        local project_choice
        read -e -p "$(echo -e "${gl_bufan}请输入（${gl_huang}序号${gl_bufan}）选择: ${gl_bai}")" project_choice
...
show_compose_commands_menu() {
    ...
    while true; do
        ...
        # 使用横向排列显示命令选项
        echo -e "${gl_huang}${gl_bufan}1. ${gl_bai}启动${gl_huang}$current_dir_name${gl_bai}服务        ${gl_huang}${gl_bufan}2. ${gl_bai}停止${gl_huang}$current_dir_name${gl_bai}服务        ${gl_huang}${gl_bufan}3. ${gl_bai}重启${gl_huang}$current_dir_name${gl_bai}服务"
        echo -e "${gl_huang}${gl_bufan}4. ${gl_bai}更新${gl_huang}$current_dir_name${gl_bai}容器        ${gl_huang}${gl_bufan}5. ${gl_bai}查看${gl_huang}$current_dir_name${gl_bai}配置        ${gl_huang}${gl_bufan}6. ${gl_bai}编辑${gl_huang}$current_dir_name${gl_bai}配置"
        echo -e "${gl_huang}${gl_bufan}7. ${gl_bai}${gl_huang}$current_dir_name${gl_bai}服务状态        ${gl_huang}${gl_bufan}8. ${gl_bai}${gl_huang}$current_dir_name${gl_bai}服务日志        ${gl_huang}${gl_bufan}9. ${gl_bai}重新构建${gl_huang}$current_dir_name${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}0. ${gl_bai}返回项目列表"
        echo -e "${gl_bufan}------------------------${gl_bai}"
...
git_clone_docker_projects() {
    ...
    while true; do
        ...
        echo -e "${gl_bufan}3.   ${gl_bai}博客系统halo                 ${gl_bufan}4.   ${gl_bai}博客系统hexo${gl_bai}"
        echo -e "${gl_bufan}5.   ${gl_bai}云文档md                     ${gl_bufan}6.   ${gl_bai}配置编辑${gl_bai}"
        echo -e "${gl_bufan}7.   ${gl_bai}文档管理mindoc               ${gl_bufan}8.   ${gl_bai}爱盼影视${gl_bai}"
        echo -e "${gl_bufan}9.   ${gl_bai}影视聚合libretv              ${gl_bufan}10.  ${gl_bai}影视聚合moontv${gl_bai}"
        echo -e "${gl_bufan}11.  ${gl_bai}影视刮削nastools             ${gl_bufan}12.  ${gl_bai}媒体服务emby${gl_bai}"
        echo -e "${gl_bufan}13.  ${gl_bai}电视助手tvhelper             ${gl_bufan}14.  ${gl_bai}音乐下载musicn${gl_bai}"
        echo -e "${gl_bufan}15.  ${gl_bai}音乐播放navidrome            ${gl_bufan}16.  ${gl_bai}小米音乐xiaomusic${gl_bai}"
        echo -e "${gl_bufan}17.  ${gl_bai}下载器xunlei                 ${gl_bufan}18.  ${gl_bai}下载器qbittorrent${gl_bai}"
        echo -e "${gl_bufan}19.  ${gl_bai}下载器transmission           ${gl_bufan}20.  ${gl_bai}视频下载metube${gl_bai}"
        echo -e "${gl_bufan}21.  ${gl_bai}网盘搜索cloud-saver          ${gl_bufan}22.  ${gl_bai}网盘搜索pansou${gl_bai}"
        echo -e "${gl_bufan}23.  ${gl_bai}网盘挂载openlist             ${gl_bufan}24.  ${gl_bai}文件服务nginx-file${gl_bai}"
        echo -e "${gl_bufan}25.  ${gl_bai}文件服务dufs                 ${gl_bufan}26.  ${gl_bai}云盘同步taosync${gl_bai}"
        echo -e "${gl_bufan}27.  ${gl_bai}导航面板sun-panel            ${gl_bufan}28.  ${gl_bai}导航面板helper${gl_bai}"
        echo -e "${gl_bufan}29.  ${gl_bai}工具箱ittools                ${gl_bufan}30.  ${gl_bai}随机图片random-pic-api${gl_bai}"
        echo -e "${gl_bufan}31.  ${gl_bai}思维导图mind-map             ${gl_bufan}32.  ${gl_bai}路由系统istoreos${gl_bai}"
        echo -e "${gl_bufan}33.  ${gl_bai}网络加速kspeeder             ${gl_bufan}34.  ${gl_bai}网站监控uptime-kuma${gl_bai}"
        echo -e "${gl_bufan}35.  ${gl_bai}内网测速speedtest            ${gl_bufan}36.  ${gl_bai}语音文字easyvoice${gl_bai}"
        echo -e "${gl_bufan}37.  ${gl_bai}容器更新watchtower           ${gl_bufan}38.  ${gl_bai}格式转换reubah${gl_bai}"
        echo -e "${gl_bufan}39.  ${gl_bai}代码托管gitea                ${gl_bufan}40.  ${gl_bai}远程桌面ubuntu${gl_bai}"
        echo -e "${gl_bufan}41.  ${gl_bai}远程桌面alpine               ${gl_bufan}42.  ${gl_bai}终端工具easynode${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
...
linux_fnos_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}内网 IP 地址: ${gl_huang}$(get_internal_ip)${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}1.  ${gl_bai}Compose容器管理   ${gl_huang}★${gl_bai}"
        echo -e "${gl_bufan}------------------------"
        echo -e "${gl_huang}2.  ${gl_bai}Docker全局状态    ${gl_huang}★${gl_bai}"
        echo -e "${gl_bufan}------------------------"
        echo -e "${gl_huang}3.  ${gl_bai}Docker容器管理    ${gl_huang}★${gl_bai}"
        echo -e "${gl_huang}4.  ${gl_bai}Docker镜像管理"
        echo -e "${gl_huang}5.  ${gl_bai}Docker网络管理"
        echo -e "${gl_huang}6.  ${gl_bai}Docker卷管理"
        echo -e "${gl_bufan}------------------------"
        echo -e "${gl_huang}7.  ${gl_bai}清理无用的docker容器和镜像网络数据卷"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}9.  ${gl_bai}克隆Docker项目"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}0.  ${gl_bai}返回主菜单"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        read -e -p "请输入你的选择: " sub_choice
        ...
        case $sub_choice in
            1)
                ...
                if ! command -v docker &> /dev/null; then
                    ...
                    read -p "$(echo -e "${gl_bufan}是否立即安装 Docker？(y/n): ${gl_bai}")" install_docker
                    ...
                    if [ "$install_docker" = "y" ] || [ "$install_docker" = "Y" ]; then
                        ...
                        if bash <(curl -sL kejilion.sh) docker install; then
                            ...
                            read -p "$(echo -e "${gl_bufan}按任意键返回主菜单...${gl_bai}")"
                            continue
                        fi
                    else
...
linux_nginx_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}内网 IP 地址: ${gl_huang}$(get_internal_ip)${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}1.  ${gl_bai}停止Nginx"
        echo -e "${gl_huang}2.  ${gl_bai}启动Nginx"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}3.  ${gl_bai}测试Nginx配置"
        echo -e "${gl_huang}4.  ${gl_bai}测试Nginx并重启"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}5.  ${gl_bai}查看 80 端口占用"
        echo -e "${gl_huang}6.  ${gl_bai}查看Nginx运行状态"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}7.  ${gl_bai}管理conf.d目录 ${gl_huang}★${gl_bai}"
        echo -e "${gl_huang}8.  ${gl_bai}管理html目录"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}9.  ${gl_bai}配置个人网站脚本   ${gl_huang}★${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}0.  ${gl_bai}返回主菜单"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        ...
        case $choice in
            1)
                ...
                if ! command -v nginx &> /dev/null && ! systemctl is-active --quiet nginx; then
                    echo -e "${gl_huang}错误：系统未安装Nginx或Nginx服务不存在。${gl_bai}"
                    echo -e "${gl_bufan}请先安装Nginx后再执行此操作。${gl_bai}"
                    echo -e "${gl_bufan}------------------------${gl_bai}"
                    read -p "按任意键返回..." 
                    continue
                fi
            ...
            5)
                ...
                if ! sudo ss -tulnp | grep -q ":80 "; then
                    echo -e "${gl_huang}没有发现任何进程监听80端口。${gl_bai}"
                    echo -e "${gl_bufan}------------------------${gl_bai}"
                    read -p "按任意键返回..." 
                    continue
                fi
                echo -e "${gl_bufan}端口占用情况：${gl_bai}"
...
linux_git_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}内网 IP 地址: ${gl_huang}$(get_internal_ip)${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}1.  ${gl_bai}推送更新"
        echo -e "${gl_huang}2.  ${gl_bai}拉取更新"
        echo -e "${gl_huang}3.  ${gl_bai}Git项目管理工具"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}4.  ${gl_bai}Gitee配置SSH"
        echo -e "${gl_huang}5.  ${gl_bai}GitHub配置SSH"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}6.  ${gl_bai}Gitee新仓库初始化"
        echo -e "${gl_huang}7.  ${gl_bai}GitHub新仓库初始化"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}8.  ${gl_bai}修改为ssh连接"
        echo -e "${gl_huang}9.  ${gl_bai}修改为https连接"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}0.  ${gl_bai}返回主菜单"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        read -e -p "请输入你的选择: " choice
        ...
        case $choice in
            1)
                # 推送更新
                if ! command -v git &> /dev/null; then
                    echo -e "${gl_huang}系统未安装 Git，是否立即安装？(y/n)${gl_bai}"
                    read -p "请输入: " install_choice
                    if [ "$install_choice" = "y" ] || [ "$install_choice" = "Y" ]; then
                        install_git
                        if [ $? -ne 0 ]; then
                            echo -e "${gl_hong}Git 安装失败，脚本无法继续执行${gl_bai}"
                            exit 1
                        fi
                    else
                        echo -e "${gl_hong}Git 未安装，脚本无法继续执行${gl_bai}"
                        exit 1
                    fi
                fi
                # 检查当前目录是否为 Git 仓库
                if [ ! -d ".git" ] || ! git rev-parse --git-dir > /dev/null 2>&1; then
                    ...
                    echo -e "${gl_bufan}------------------------${gl_bai}"
                    read -p "$(echo -e "${gl_bufan}按任意键返回上一级菜单...${gl_bai}")"
                    continue
                fi
                clear
...
EOF
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --compress) compress_file="$2"; shift 2 ;;
            --extract) extract_file="$2"; shift 2 ;;
            --format) compress_format="$2"; shift 2 ;;
...
docker_image_pack() {
    ...
    while true; do
        echo -e "${gl_bufan}------------------------${gl_bai}"
        read -erp "$(echo -e "${gl_bufan}请输入打包的镜像名称和版本（格式 ${gl_huang}nginx:tag${gl_bai}，输入 ${gl_huang}0${gl_bai} 返回上级菜单）：") " IMG
        
        # 检查是否要返回上一级菜单
        if [ "$IMG" = "0" ]; then
            echo -e "${gl_huang}返回上一级菜单${gl_bai}"
            set +euo pipefail
            return 0
        fi
        ...
        if docker image inspect "$IMG" &>/dev/null; then
            ...
            echo -e "${gl_hong}镜像 '$IMG' 不存在，请重新输入！${gl_bai}"
        fi
    done

    # 3. 循环选择格式
    while true; do
        read -erp "$(echo -e "${gl_bufan}选择打包格式 ${gl_huang}1${gl_bai} .tar  ${gl_huang}2${gl_bai} .tar.gz（输入 ${gl_huang}1${gl_bai} 或 ${gl_huang}2${gl_bai}，输入 ${gl_huang}0${gl_bai} 返回上级菜单）：") " FMT
        ...
        case "$FMT" in
            1) EXT="tar"; break ;;
            2) EXT="tar.gz"; break ;;
            *) echo -e "${gl_hong}输入错误，请重新选择！${gl_bai}" ;;
        esac
    done

    # 4. 循环输入文件名（不含扩展名）
    while true; do
        read -erp "$(echo -e "${gl_bufan}请输入打包文件名（${gl_huang}不含扩展名${gl_bai}，输入 ${gl_huang}0${gl_bai} 返回上级菜单${gl_bai}）：") " FNAME
        ...
        # 检查输入是否为空
        if [ -z "$FNAME" ]; then
            echo -e "${gl_hong}文件名不能为空，请重新输入！${gl_bai}"
            continue
        fi
        
        if [[ "$FNAME" =~ [^a-zA-Z0-9._-] ]]; then
            echo -e "${gl_hong}文件名含非法字符，请重新输入！${gl_bai}"
        else
            # 检查文件是否已存在
            if [ -f "${FNAME}.${EXT}" ]; then
                echo -e "${gl_huang}文件 ${FNAME}.${EXT} 已存在，是否覆盖？${gl_bai}"
                read -erp "$(echo -e "${gl_bufan}输入 y 覆盖，输入 n 重新输入文件名：${gl_bai}") " OVERWRITE
...
echo -e "${gl_bufan}命令行输入${gl_huang}m up${gl_bufan}强制更新脚本${gl_bai}"
echo -e "mobufan 脚本工具箱 ${gl_huang}v$sh_v"
echo -e "${gl_bufan}------------------------${gl_bai}"
echo -e "${gl_bufan}1.   ${gl_bai}系统信息查询"
...
echo -e "${gl_bufan}------------------------${gl_bai}"
echo -e "${gl_huang}9.   ${gl_bai}PVE命令   ${gl_huang}★${gl_bai}"
echo -e "${gl_huang}10.  ${gl_bai}FnOS命令"
echo -e "${gl_huang}11.  ${gl_bai}Nginx命令"
echo -e "${gl_huang}12.  ${gl_bai}Git脚本"
echo -e "${gl_bufan}------------------------${gl_bai}"
...
echo "启动脚本            m"
echo "说明文档            m h| m 说明"
echo "安装软件包          m install nano wget | m add nano wget | m 安装 nano wget"
echo "卸载软件包          m remove nano wget | m del nano wget | m uninstall nano wget | m 卸载 nano wget"
echo "更新系统            m update | m 更新"
echo "更新脚本            m up"
echo "文件管理            m f| m file| m 文件管理"
echo "压缩解压            m zip | m gz | m rar"
echo "文件压缩            m ys | m 压缩"
echo "文件解压            m jy | m 解压"
echo "文件删除            m sc | m 删除| m 删除文件"
echo "清理系统垃圾        m clean | m 清理"
echo "重装系统面板        m dd | m 重装"
echo "bbr3控制面板        m bbr3 | m bbrv3"
echo "内核调优面板        m nhyh | m 内核优化"
echo "设置虚拟内存        m swap 2048"
echo "设置虚拟时区        m time Asia/Shanmhai | m 时区 Asia/Shanghai"
echo "系统回收站          m trash | m hsz | m 回收站"
echo "系统备份功能        m bacgup | m bf | m 备份"
echo "ssh远程连接工具     m ssh | m 远程连接"
echo "rsync远程同步工具   m rsync | m 远程同步"
echo "硬盘管理工具        m disg | m 硬盘管理"
echo "内网穿透（服务端）  m frps"
echo "内网穿透（客户端）  m frpc"
...
echo "软件开机启动        m enable docger | m autostart docge | m 开机启动 docger "
echo "域名证书申请        m ssl"
echo "域名证书到期查询    m ssl ps"
echo "docger管理平面      m docger"
echo "docger环境安装      m docger install |m docger 安装"
echo "docger容器管理      m docger ps |m docger 容器"
echo "docger镜像管理      m docger imm |m docger 镜像"
echo "LDNMP站点管理       m web"
echo "LDNMP缓存清理       m web cache"
echo "安装WordPress       m wp |m wordpress |m wp xxx.com"
echo "安装反向代理        m fd |m rp |m 反代 |m fd xxx.com"
echo "安装负载均衡        m loadbalance |m 负载均衡"
echo "安装L4负载均衡      m stream |m L4负载均衡"
echo "防火墙面板          m fhq |m 防火墙"
echo "开放端口            m dgdg 8080 |m 打开端口 8080"
echo "关闭端口            m gbdg 7800 |m 关闭端口 7800"
