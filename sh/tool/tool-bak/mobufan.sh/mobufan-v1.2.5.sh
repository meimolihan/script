...
linux_file() {
	...
	while true; do
		...
		echo "------------------------"
		echo "11. 创建文件        12. 编辑文件          13. 创建并编辑文件    14. 查看文件内容"
		echo "15. 删除文件        16. 重命名文件        17. 修改文件权限"
		echo "------------------------"
...
pve_shutdown_selector() {
    ...
    get_running_instances() {
       RUNNING_INSTANCES=()
        ...
        INSTANCE_IDS=()
        INSTANCE_NAMES=()
        echo -e "${gl_lan}正在获取运行中的实例...${gl_bai}"
...
show_running_instances() {
    ...
    while :; do
        read -p "$(echo -e "${gl_huang}请输入要关闭的实例序号 (输入 0 返回): ${gl_bai}")" selection
        case "$selection" in
            0)
            ...
            *)
                ...
                if [ "$index" -ge 0 ] && [ "$index" -lt "${#RUNNING_INSTANCES[@]}" ]; then
                    ...
                    echo -e "${gl_hong}错误: 序号 ${selection} 不在有效范围内${gl_bai}"
                fi
                ;;
        esac
    done
...
pve_start_selector() {
    ...
    get_stopped_instances() {
        ...
        INSTANCE_IDS=()
        INSTANCE_NAMES=()
        echo -e "${gl_lan}正在获取已停止的实例...${gl_bai}"
    ...
    while :; do
        read -p "$(echo -e "${gl_huang}请输入要启动的实例序号 (输入 0 返回): ${gl_bai}")" selection
        case "$selection" in
            0)
                return 0        # 用 return 直接跳出函数
                ;;
...
pve_restart_selector() {
    # 获取所有实例信息函数
    get_all_instances() {
        echo -e "${gl_lan}正在获取所有实例信息...${gl_bai}"
        ...
        for ((i=0; i<total_instances; i+=instances_per_line)); do
            for ((j=0; j<instances_per_line; j++)); do
                ...
                if [ $index -lt $total_instances ]; then
                    ...
                    # 分别打印各个部分，避免颜色代码影响对齐
                    printf "${gl_huang}%2d${gl_bai}) ${status_color}${status_symbol}${gl_bai}${type_str}:${gl_huang}${INSTANCE_IDS[$index]}${gl_bai}"
    ...
    main_loop() {
        while true; do
            ...
            # 用户选择
            echo
            read -p "$(echo -e "${gl_huang}请输入要操作的实例序号 (输入 0 返回上级菜单): ${gl_bai}")" selection
            
            case "$selection" in
                0)
                    echo -e "${gl_huang}返回上级菜单${gl_bai}"
                    break
                    ;;
                *)
                    # 检查输入是否为数字且在有效范围内
...
linux_pve_menu() {
	while true; do
	  ...
	  echo -e "PVE 命令"
          ...
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_bufan}2.   ${gl_bai}启动虚拟机"
          echo -e "${gl_bufan}3.   ${gl_bai}关闭虚拟机"
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_bufan}4.   ${gl_bai}启动LXC容器"
          echo -e "${gl_bufan}5.   ${gl_bai}关闭LXC容器"
          echo -e "${gl_bufan}------------------------${gl_bai}"
          echo -e "${gl_bufan}6.   ${gl_bai}管理备份目录"
          echo -e "${gl_bufan}7.   ${gl_bai}管理固件目录"
          echo -e "${gl_bufan}------------------------${gl_bai}"
	  ...
	  case $sub_choice in
                         ...
                         if safe_read "请输入要启动的虚拟机ID(多个用空格隔开，输入q退出)" vm_ids; then
                             if [ -n "$vm_ids" ]; then
                                  for vm_id in $vm_ids; do
                                     echo -e "${gl_bufan}启动虚拟机: $vm_id${NC}"
                                     qm start $vm_id
                                 done
                             else
                                 echo -e "${gl_huang}错误: 未输入虚拟机ID${NC}"
                             fi
                         fi
                         break_end
                         ;;
		  3)
                        ...
                        if safe_read "请输入要关闭的虚拟机ID(多个用空格隔开，输入q退出)" vm_ids; then
                            if [ -n "$vm_ids" ]; then
                                for vm_id in $vm_ids; do
                                    echo -e "${gl_bufan}关闭虚拟机: $vm_id${NC}"
                                    qm stop $vm_id
                                done
                            else
                                echo -e "${gl_huang}错误: 未输入虚拟机ID${NC}"
                        ...
                        if safe_read "请输入要启动的容器ID(多个用空格隔开，输入q退出)" ct_ids; then
                            if [ -n "$ct_ids" ]; then
                                for ct_id in $ct_ids; do
                                    echo -e "${gl_bufan}启动容器: $ct_id${NC}"
                                    lxc-start -n $ct_id
                                done
                            else
                                echo -e "${gl_huang}错误: 未输入容器ID${NC}"
                            fi
                        fi
                        break_end
                        ;;
...
show_compose_project_menu() {
    ...
    while true; do
        ...
        read -e -p "$(echo -e "${gl_bufan}请输入（序号）选择: ${gl_bai}")" project_choice
