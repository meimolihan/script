...
install() {
    ...
    : "${gl_huang:="\033[33m"}" "${gl_bai:="\033[0m"}"
    ...
    for pkg in "$@"; do
        ...
        for mgr in dnf yum apt apk pacman zypper opkg pkg; do
            case $mgr in
                dnf)  command -v dnf &>/dev/null && { dnf -y update; dnf install -y epel-release; dnf install -y "$pkg"; continue 2; };;
                yum)  command -v yum &>/dev/null && { yum -y update; yum install -y epel-release; yum install -y "$pkg"; continue 2; };;
                apt)  command -v apt &>/dev/null && { apt update -y; apt install -y "$pkg"; continue 2; };;
                apk)  command -v apk &>/dev/null && { apk update; apk add "$pkg"; continue 2; };;
                pacman) command -v pacman &>/dev/null && { pacman -Syu --noconfirm; pacman -S --noconfirm "$pkg"; continue 2; };;
                zypper) command -v zypper &>/dev/null && { zypper refresh; zypper install -y "$pkg"; continue 2; };;
                opkg) command -v opkg &>/dev/null && { opkg update; opkg install "$pkg"; continue 2; };;
                pkg)  command -v pkg &>/dev/null && { pkg update; pkg install -y "$pkg"; continue 2; };;
            esac
        done
    done
}
...
ldnmp_v() {
    ...
    # 获取mysql版本
    local dbrootpasswd=$(grep -oP 'MYSQL_ROOT_PASSWORD:\s*\K.*' /etc/nginx/docker-compose.yml | tr -d '[:space:]')
...
default_server_ssl() {
    ...
    if command -v dnf &>/dev/null || command -v yum &>/dev/null; then
        openssl req -x509 -nodes -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 -keyout /etc/nginx/certs/default_server.key -out /etc/nginx/certs/default_server.crt -days 5475 -subj "/C=US/ST=State/L=City/O=Organization/OU=Organizational Unit/CN=Common Name"
    else
        openssl genpkey -algorithm Ed25519 -out /etc/nginx/certs/default_server.key
        openssl req -x509 -key /etc/nginx/certs/default_server.key -out /etc/nginx/certs/default_server.crt -days 5475 -subj "/C=US/ST=State/L=City/O=Organization/OU=Organizational Unit/CN=Common Name"
    fi
...
web_del() {
    ...
    for yuming in "$yuming"_list; do
        ...
        rm "/etc/nginx/certs/${yuming}_key.pem" >/dev/null 2>&1
        rm "/etc/nginx/certs/${yuming}_cert.pem" >/dev/null 2>&1
...
nginx_br() {
    ...
    if ! grep -q "kjlion/nginx:alpine" /etc/nginx/docker-compose.yml; then
        # wget -O /etc/nginx/nginx.conf "${gh_proxy}raw.githubusercontent.com/kejilion/nginx/main/nginx10.conf"
        wget -O /etc/nginx/nginx.conf "https://gitee.com/meimolihan/fail2ban/raw/master/nginx/nginx.conf"
    fi
...
web_security() {
    while true; do
        ...
        1)
            ...
            # curl -sS -O https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/fail2ban-nginx-cc.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/fail2ban-nginx-cc.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/nginx-418.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/nginx-403.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/nginx-deny.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/nginx-unauthorized.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/nginx-bad-request.conf
            wget https://gitee.com/meimolihan/fail2ban/raw/master/filter.d/nginx-pve.conf
            ...
            curl -sS -O https://gitee.com/meimolihan/fail2ban/raw/master/jail.d/nginx-cc.conf
...
f2b_autostart(){
    ...
    if [[ $status == "enabled" ]]; then
        echo -e "\n[✔] 无需重复操作，${gl_lv}fail2ban 已是开机自启状态。${gl_bai}"
    else
        echo -e "\n正在设置 fail2ban 开机自启，请稍候..."
        if systemctl enable fail2ban >/dev/null 2>&1; then
            echo -e "\n[✔] 设置成功！fail2ban 已设为开机自启。"
        else
            echo -e "\n[✘] 设置失败，请检查是否安装 fail2ban 或以 root 身份运行。"
        fi
    fi
}
...
ldnmp_web_status() {
    ...
    while true; do
        local cert_count
        cert_count=$(find /etc/nginx/certs/ -name "*_cert.pem" -type f 2>/dev/null | wc -l)
        local output
        ...
        echo -e "------------------------"
        for cert_file in /etc/nginx/certs/*_cert.pem; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "数据 ${gl_hui}/etc/nginx/html${gl_bai}     证书 ${gl_hui}/etc/nginx/certs${gl_bai}     配置 ${gl_hui}/etc/nginx/conf.d${gl_bai}"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo ""
        echo "操作"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        ...
        2)
            ...
            # rm /etc/nginx/certs/${oddyuming}_key.pem
            # rm /etc/nginx/certs/${oddyuming}_cert.pem
        ...
        4)
            ...
            sed -i "s|/etc/nginx/certs/${oddyuming}_cert.pem|/etc/nginx/certs/${yuming}_cert.pem|g" /etc/nginx/conf.d/"$yuming".conf
            sed -i "s|/etc/nginx/certs/${oddyuming}_key.pem|/etc/nginx/certs/${yuming}_key.pem|g" /etc/nginx/conf.d/"$yuming".conf

            docker exec nginx nginx -s reload

            ;;
        5)
            tail -n 200 /etc/nginx/log/nginx/access.log
            break_end
            ;;
        6)
            tail -n 200 /etc/nginx/log/nginx/error.log
            break_end
            ;;
        7)
            install nano
        ...
        10)
            install goaccess
            goaccess --log-format=COMBINED /etc/nginx/log/nginx/access.log
            ;;
...
ldnmp_tato() {
    local cert_count=$(ls /etc/nginx/certs/*_cert.pem 2>/dev/null | wc -l)
    local output="${gl_lv}${cert_count}${gl_bai}"

    local dbrootpasswd=$(grep -oP 'MYSQL_ROOT_PASSWORD:\s*\K.*' /etc/nginx/docker-compose.yml 2>/dev/null | tr -d '[:space:]')
...
linux_git_menu() {
    while true; do
        ...
        1)
            # 推送更新
            if ! command -v git &>/dev/null; then
                echo -e "${gl_huang}系统未安装 Git，是否立即安装？(${gl_lv}y${gl_bai}/${gl_hong}N${gl_bai})${gl_bai}"
...
return
        ...
        2)
            # 拉取更新
            if ! command -v git &>/dev/null; then
                echo -e "${gl_huang}系统未安装 Git，是否立即安装？(${gl_lv}y${gl_bai}/${gl_hong}N${gl_bai})"
                read -r -e -p "请输入: " install_choice
                if [ "$install_choice" = "y" ] || [ "$install_choice" = "Y" ]; then
                    install_git
                    if [ $? -ne 0 ]; then
...
mobufan_sh() {
    while true; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_huang}10.  ${gl_bai}PVE命令"
        echo -e "${gl_bufan}11.  ${gl_bai}FnOS命令"
        echo -e "${gl_bufan}12.  ${gl_bai}Nginx命令"
        echo -e "${gl_bufan}13.  ${gl_bai}Git脚本"
        echo -e "${gl_bufan}------------------------${gl_bai}"
...
m_info() {
    ...
    echo -e "${gl_bufan}安装软件${gl_bai}            ${gl_huang}m install nano wget ${gl_hong}| ${gl_huang}m add nano wget ${gl_hong}| ${gl_huang}m 安装 nano wget${gl_bai}"
    echo -e "${gl_bufan}卸载软件${gl_bai}            ${gl_huang}m remove nano wget ${gl_hong}| ${gl_huang}m del nano wget ${gl_hong}| ${gl_huang}m uninstall nano wget ${gl_hong}| ${gl_huang}m 卸载 nano wget${gl_bai}"
    ...
    echo -e "${gl_bufan}文件下载${gl_bai}            ${gl_huang}m xz ${gl_hong}| ${gl_huang}m 下载 ${gl_hong}| ${gl_huang}m xz <URL>${gl_bai}"
    echo -e "${gl_bufan}------------------------------------------------${gl_bai}"
