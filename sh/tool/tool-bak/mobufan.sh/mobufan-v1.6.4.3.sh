...
linux_file() {
    ...
    while true; do
        ...
        echo -e "${gl_bufan}17. ${gl_bai}创建并编辑文件"
        echo -e "${gl_bufan}------------------------------------------------${gl_bai}"
        echo -e "${gl_bufan}21. ${gl_bai}压缩文件目录    ${gl_bufan}22. ${gl_bai}解压文件目录       ${gl_bufan}23. ${gl_bai}解压压缩工具"
        echo -e "${gl_bufan}24. ${gl_bai}移动文件目录    ${gl_bufan}25. ${gl_bai}复制文件目录       ${gl_bufan}26. ${gl_bai}模糊搜索文件"
        echo -e "${gl_bufan}------------------------------------------------${gl_bai}"
        echo -e "${gl_bufan}27. ${gl_bai}文件下载工具    ${gl_bufan}28. ${gl_bai}传送文件至远端服务器"
        echo -e "${gl_bufan}------------------------------------------------${gl_bai}"
...
download_file() {
    ...
    if [[ -n "$1" ]]; then
        ...
        while true; do
            ...
            read -r -e -p "$(echo -e "${gl_bai}请输入要下载的文件链接（输入 ${gl_huang}q${gl_bai} 退出）：")" url
            [[ "$url" == "q" || "$url" == "Q" ]] && echo -e "${gl_hui}退出下载功能。${gl_bai}" && return
