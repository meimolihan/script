...
show_compose_commands_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}4.   ${gl_bai}更新容器"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_bufan}5.   ${gl_bai}查看配置文件"
        echo -e "${gl_bufan}6.   ${gl_bai}编辑配置文件"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_bufan}7.   ${gl_bai}查看服务状态"
        echo -e "${gl_bufan}8.   ${gl_bai}查看服务日志"
        echo -e "${gl_bufan}9.   ${gl_bai}重新构建并启动"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        ...
        case $cmd_choice in
            ...
            4)
                # 更新容器
                echo -e "${gl_bufan}正在更新当前容器...${gl_bai}"
                docker-compose down && docker-compose pull && docker-compose up -d && docker image prune -f
	        break_end
                ;;
            5)
	...
	while true; do
	  ...
	  case $sub_choice in
		  1)
                         ...
                         clear
                         cd /vol1/1000/compose && show_compose_project_menu
                         ;;
		  2)
			  clear
			  local container_count=$(docker ps -a -q 2>/dev/null | wc -l)
...
safe_read() {
    ...
    while true; do
        ...
        # 处理空输入
        if [[ -z "${!var_name}" ]]; then
            echo -e "${gl_huang}输入为空，请重新输入${gl_bai}"
            continue
        fi
        ...
        case "$validation" in
            number)
                if [[ ! "${!var_name}" =~ ^[0-9]+$ ]]; then
                    echo -e "${gl_hong}请输入有效的数字${gl_bai}"
                    continue
                fi
                ;;
            file)
                if [[ ! -f "${!var_name}" ]]; then
                    echo -e "${gl_hong}文件不存在，请重新输入${gl_bai}"
                    continue
                fi
                ;;
            dir)
                if [[ ! -d "${!var_name}" ]]; then
                    echo -e "${gl_hong}目录不存在，请重新输入${gl_bai}"
                    continue
                fi
                ;;
            any)
...
echo "压缩解压            m zip|m gz|m rar|m 压缩|m 解压"
