...
ldnmp_web_status() {
    root_use
    while true; do
        # ① 证书数（*.pem 在 /etc/nginx/keyfile/）
        local cert_count
        cert_count=$(find /etc/nginx/keyfile/ -name "*_cert.pem" -type f 2>/dev/null | wc -l)
        ...
        echo -e "------------------------"
        for cert_file in /etc/nginx/keyfile/*.pem; do
            [ -f "$cert_file" ] || continue
            local domain=$(basename "$cert_file" .pem)   # 去掉 .pem 即域名
        ...
        7)
            ...
            ;;
        8)
            read -rp "编辑站点配置，请输入你要编辑的域名: " yuming
