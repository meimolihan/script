...
view_nginx_status(){
    ...
    if ! command -v systemctl &>/dev/null; then
        ...
        if pgrep -x nginx &>/dev/null; then
            echo -e "${gl_bai}状态：${gl_lv}已启动${gl_bai}"
        else
            echo -e "${gl_bai}状态：${gl_hui}已停止${gl_bai}"
        fi
        return
    fi
    ...
    case "$active" in
        active)  echo -e "${gl_bai}状态：${gl_lv}已启动${gl_bai}   ${gl_bai}版本: ${gl_lv}$ver${gl_bai}" ;;
        inactive|failed|*) echo -e "${gl_bai}状态：${gl_hui}已停止${gl_bai}   ${gl_bai}版本: ${gl_lv}$ver${gl_bai}" ;;
    esac
}
