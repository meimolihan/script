...
cp -f ~/mobufan.sh /usr/local/bin/g > /dev/null 2>&1
...
UserLicenseAgreement() {
	clear
	echo -e "${gl_bufan}欢迎使用科技lion脚本工具箱${gl_bai}"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_bufan}101. ${gl_bai}k命令高级用法 ${gl_huang}★${gl_bai}                    ${gl_bufan}102. ${gl_bai}卸载mobufan脚本"
	  echo -e "${gl_bufan}------------------------"
	  echo -e "${gl_bufan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_bufan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
linux_nginx_menu() {
    while true; do
        ...
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_bufan}7.   ${gl_bai}查看Nginx配置目录"
        echo -e "${gl_bufan}8.   ${gl_bai}查看html目录"
        echo -e "${gl_bufan}9.   ${gl_bai}查看Nginx版本"
        echo -e "${gl_bufan}------------------------${gl_bai}"
        echo -e "${gl_bufan}0.   ${gl_bai}返回主菜单"
        echo -e "${gl_bufan}------------------------${gl_bai}"
...
echo -e "${gl_bufan}命令行输入${gl_huang}g${gl_bufan}可快速启动脚本${gl_bai}"
echo -e "${gl_bufan}------------------------${gl_bai}"
...
echo -e "${gl_bufan}------------------------${gl_bai}"
echo -e "${gl_huang}9.   ${gl_bai}PVE常用命令   ${gl_huang}★${gl_bai}"
echo -e "${gl_huang}10.  ${gl_bai}FnOS常用命令"
echo -e "${gl_huang}11.  ${gl_bai}Nginx常用合集"
echo -e "${gl_huang}12.  ${gl_bai}Git脚本工具"
echo -e "${gl_bufan}------------------------${gl_bai}"
...
echo "以下是g命令参考用例："
echo "启动脚本            g"
echo "安装软件包          g install nano wget | g add nano wget | g 安装 nano wget"
echo "卸载软件包          g remove nano wget | g del nano wget | g uninstall nano wget | g 卸载 nano wget"
echo "更新系统            g update | g 更新"
echo "清理系统垃圾        g clean | g 清理"
echo "重装系统面板        g dd | g 重装"
echo "bbr3控制面板        g bbr3 | g bbrv3"
echo "内核调优面板        g nhyh | g 内核优化"
echo "设置虚拟内存        g swap 2048"
echo "设置虚拟时区        g time Asia/Shanghai | g 时区 Asia/Shanghai"
echo "系统回收站          g trash | g hsz | g 回收站"
echo "系统备份功能        g bacgup | g bf | g 备份"
echo "ssh远程连接工具     g ssh | g 远程连接"
echo "rsync远程同步工具   g rsync | g 远程同步"
echo "硬盘管理工具        g disg | g 硬盘管理"
echo "内网穿透（服务端）  g frps"
echo "内网穿透（客户端）  g frpc"
echo "软件启动            g start sshd | g 启动 sshd "
echo "软件停止            g stop sshd | g 停止 sshd "
echo "软件重启            g restart sshd | g 重启 sshd "
echo "软件状态查看        g status sshd | g 状态 sshd "
echo "软件开机启动        g enable docger | g autostart docge | g 开机启动 docger "
echo "域名证书申请        g ssl"
echo "域名证书到期查询    g ssl ps"
echo "docger管理平面      g docger"
echo "docger环境安装      g docger install |g docger 安装"
echo "docger容器管理      g docger ps |g docger 容器"
echo "docger镜像管理      g docger img |g docger 镜像"
echo "LDNMP站点管理       g web"
echo "LDNMP缓存清理       g web cache"
echo "安装WordPress       g wp |g wordpress |g wp xxx.com"
echo "安装反向代理        g fd |g rp |g 反代 |g fd xxx.com"
echo "安装负载均衡        g loadbalance |g 负载均衡"
echo "安装L4负载均衡      g stream |g L4负载均衡"
echo "防火墙面板          g fhq |g 防火墙"
echo "开放端口            g dgdg 8080 |g 打开端口 8080"
echo "关闭端口            g gbdg 7800 |g 关闭端口 7800"
echo "放行IP              g fxip 127.0.0.0/8 |g 放行IP 127.0.0.0/8"
echo "阻止IP              g zzip 177.5.25.36 |g 阻止IP 177.5.25.36"
echo "命令收藏夹          g fav | g 命令收藏夹"
echo "应用市场管理        g app"
echo "应用编号快捷管理    g app 26 | g app 1panel | g app npm"
echo "显示系统信息        g info"
}
